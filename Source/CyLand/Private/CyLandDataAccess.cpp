...
CYLAND_API FCyLandComponentDataInterface::FCyLandComponentDataInterface(UCyLandComponent* InComponent, int32 InMipLevel) :
...
CYLAND_API FCyLandComponentDataInterface::~FCyLandComponentDataInterface()
{
	if (HeightMipData)
	{
...
CYLAND_API void FCyLandComponentDataInterface::GetHeightmapTextureData(TArray<FColor>& OutData, bool bOkToFail)
{
...
CYLAND_API bool FCyLandComponentDataInterface::GetWeightmapTextureData(UCyLandLayerInfoObject* LayerInfo, TArray<uint8>& OutData)
{
...
CYLAND_API FColor* FCyLandComponentDataInterface::GetXYOffsetData(int32 LocalX, int32 LocalY) const
{
#if LANDSCAPE_VALIDATE_DATA_ACCESS
	check(Component);
...
CYLAND_API FVector FCyLandComponentDataInterface::GetLocalVertex(int32 LocalX, int32 LocalY) const
{
	...
	return FVector(LocalX * ScaleFactor + XOffset, LocalY * ScaleFactor + YOffset, CyLandDataAccess::GetLocalHeight(GetHeight(LocalX, LocalY)));
}

CYLAND_API FVector FCyLandComponentDataInterface::GetWorldVertex(int32 LocalX, int32 LocalY) const
{
	return Component->GetComponentTransform().TransformPosition(GetLocalVertex(LocalX, LocalY));
}

CYLAND_API void FCyLandComponentDataInterface::GetWorldTangentVectors(int32 LocalX, int32 LocalY, FVector& WorldTangentX, FVector& WorldTangentY, FVector& WorldTangentZ) const
{
...
CYLAND_API void FCyLandComponentDataInterface::GetWorldPositionTangents(int32 LocalX, int32 LocalY, FVector& WorldPos, FVector& WorldTangentX, FVector& WorldTangentY, FVector& WorldTangentZ) const
{
	FColor* Data = GetHeightData(LocalX, LocalY);
	...
	float XOffset, YOffset;
	GetXYOffset(LocalX, LocalY, XOffset, YOffset);
	WorldPos = Component->GetComponentTransform().TransformPosition(FVector(LocalX * ScaleFactor + XOffset, LocalY * ScaleFactor + YOffset, CyLandDataAccess::GetLocalHeight(Height)));
	WorldTangentX = Component->GetComponentTransform().TransformVectorNoScale(WorldTangentX);
	WorldTangentY = Component->GetComponentTransform().TransformVectorNoScale(WorldTangentY);
	WorldTangentZ = Component->GetComponentTransform().TransformVectorNoScale(WorldTangentZ);
}
