...
/*=============================================================================
CyLandLight.cpp: Static lighting for CyLandComponents
=============================================================================*/
...
TMap<FIntPoint, FColor> FCyLandStaticLightingMesh::CyLandUpscaleHeightDataCache;
TMap<FIntPoint, FColor> FCyLandStaticLightingMesh::CyLandUpscaleXYOffsetDataCache;

/** A texture mapping for landscapes */
/** Initialization constructor. */
FCyLandStaticLightingTextureMapping::FCyLandStaticLightingTextureMapping(
	UCyLandComponent* InComponent,FStaticLightingMesh* InMesh,int32 InLightMapWidth,int32 InLightMapHeight,bool bPerformFullQualityRebuild) :
...
void FCyLandStaticLightingTextureMapping::Apply(FQuantizedLightmapData* QuantizedData, const TMap<ULightComponent*,FShadowMapData2D*>& ShadowMapData, ULevel* LightingScenario)
{
	...
	ULevel* StorageLevel = LightingScenario ? LightingScenario : CyLandComponent->GetOwner()->GetLevel();
	UMapBuildDataRegistry* Registry = StorageLevel->GetOrCreateMapBuildData();
	FMeshMapBuildData& MeshBuildData = Registry->AllocateMeshBuildData(CyLandComponent->MapBuildDataId, true);
...
namespace
{
	...
	int32 GetLightingLOD(const UCyLandComponent* InComponent)
	{
...
/** Initialization constructor. */
FCyLandStaticLightingMesh::FCyLandStaticLightingMesh(UCyLandComponent* InComponent, const TArray<ULightComponent*>& InRelevantLights, int32 InExpandQuadsX, int32 InExpandQuadsY, float InLightMapRatio, int32 InLOD)
	: FStaticLightingMesh(
...
FCyLandStaticLightingMesh::~FCyLandStaticLightingMesh()
{
}

namespace
{
	void GetLODData(UCyLandComponent* CyLandComponent, int32 X, int32 Y, int32 HeightmapOffsetX, int32 HeightmapOffsetY, int32 LODValue, int32 HeightmapStride, FColor& OutHeight, FColor& OutXYOffset)
	{
		int32 ComponentSize = ((CyLandComponent->SubsectionSizeQuads + 1) * CyLandComponent->NumSubsections) >> LODValue;
		int32 LODHeightmapSizeX = CyLandComponent->GetHeightmap()->Source.GetSizeX() >> LODValue;
		int32 LODHeightmapSizeY = CyLandComponent->GetHeightmap()->Source.GetSizeY() >> LODValue;
		float Ratio = (float)(LODHeightmapSizeX) / (HeightmapStride);

		int32 CurrentHeightmapOffsetX = FMath::RoundToInt((float)(LODHeightmapSizeX) * CyLandComponent->HeightmapScaleBias.Z);
		int32 CurrentHeightmapOffsetY = FMath::RoundToInt((float)(LODHeightmapSizeY) * CyLandComponent->HeightmapScaleBias.W);
		...
		FCyLandComponentDataInterface DataInterface(CyLandComponent, LODValue);
		...
		if (CyLandComponent->XYOffsetmapTexture)
		{
	...
	void InternalUpscaling(FCyLandComponentDataInterface& DataInterface, UCyLandComponent* CyLandComponent, int32 InLOD, int32 GeometryLOD, TArray<FColor>& CompHeightData, TArray<FColor>& CompXYOffsetData)
	{
		// Upscaling using CyLand LOD system 
		UCyLandInfo* const Info = CyLandComponent->GetCyLandInfo();
		check(Info);

		FIntPoint ComponentBase = CyLandComponent->GetSectionBase() / CyLandComponent->ComponentSizeQuads;
		int32 NeighborLODs[8] = { 0, 0, 0, 0, 0, 0, 0, 0 };

		int32 MaxLOD = FMath::CeilLogTwo(CyLandComponent->SubsectionSizeQuads + 1) - 1;
		...
		for (int32 y = -1; y <= 1; y++)
		{
			for (int32 x = -1; x <= 1; x++)
			{
				...
				UCyLandComponent* Neighbor = Info->XYtoComponentMap.FindRef(ComponentBase + FIntPoint(x, y));
				int32 NeighborLOD = -1;
				...
				else
				{
					...
					for (int32 yy = -1; yy <= 1; yy++)
					{
						for (int32 xx = -1; xx <= 1; xx++)
						{
							...
							UCyLandComponent* ComponentNeighbor = Info->XYtoComponentMap.FindRef(ComponentBase + FIntPoint(x+xx, y+yy));
		...
		if (bNeedUpscaling)
		{
			check(CyLandComponent);
			// Need Upscaling
			int32 HeightmapStride = CyLandComponent->GetHeightmap()->Source.GetSizeX() >> InLOD;
			int32 HeightDataSize = HeightmapStride * CyLandComponent->GetHeightmap()->Source.GetSizeY() >> InLOD;
			...
			int32 ComponentSize = ((CyLandComponent->SubsectionSizeQuads + 1) * CyLandComponent->NumSubsections) >> InLOD;
			...
			for (int32 Y = DataInterface.HeightmapComponentOffsetY; Y < DataInterface.HeightmapComponentOffsetY + ComponentSize; ++Y)
			{
				for (int32 X = DataInterface.HeightmapComponentOffsetX; X < DataInterface.HeightmapComponentOffsetX + ComponentSize; ++X)
				{
					...
					FColor* CachedHeight = FCyLandStaticLightingMesh::CyLandUpscaleHeightDataCache.Find(IXY);
					FColor* CachedXYOffset = FCyLandStaticLightingMesh::CyLandUpscaleXYOffsetDataCache.Find(IXY);
					...
					else
					{
						...
						FColor Height[2];
						FColor XYOffset[2];
						::GetLODData(CyLandComponent, X, Y, DataInterface.HeightmapComponentOffsetX, DataInterface.HeightmapComponentOffsetY,
						...
						FCyLandStaticLightingMesh::CyLandUpscaleHeightDataCache.Add(IXY, CompHeightData[X + Y * HeightmapStride]);
						if (CyLandComponent->XYOffsetmapTexture)
						{
							FCyLandStaticLightingMesh::CyLandUpscaleHeightDataCache.Add(IXY, CompXYOffsetData[X + Y * HeightmapStride]);
						}
					}
				}
			}
...
void FCyLandStaticLightingMesh::GetHeightmapData(int32 InLOD, int32 GeometryLOD)
{
	UCyLandInfo* const Info = CyLandComponent->GetCyLandInfo();
	check(Info);

	bool bUseRenderedWPO = CyLandComponent->GetCyLandProxy()->bUseMaterialPositionOffsetInStaticLighting &&
	                       CyLandComponent->GetCyLandMaterial()->GetMaterial()->WorldPositionOffset.IsConnected();

	HeightData.Empty(FMath::Square(NumVertices));
	HeightData.AddUninitialized(FMath::Square(NumVertices));

	const int32 NumSubsections = CyLandComponent->NumSubsections;
	const int32 SubsectionSizeVerts = (CyLandComponent->SubsectionSizeQuads + 1) >> InLOD;
	...
	// copy heightmap data for this component...
	{
		...
		FCyLandComponentDataInterface DataInterface(CyLandComponent, InLOD);
		::InternalUpscaling(DataInterface, CyLandComponent, InLOD, GeometryLOD, CompHeightData, CompXYOffsetData);

		TArray<uint16> RenderedWPOData;
		if (bUseRenderedWPO)
		{
			// todo - do we need to invalidate the landscape mesh version?
			RenderedWPOData = CyLandComponent->RenderWPOHeightmap(InLOD);
		}
	...
	for (int32 ComponentY = -1; ComponentY <= 1; ++ComponentY)
	{
		for (int32 ComponentX = -1; ComponentX <= 1; ++ComponentX)
		{
			...
			UCyLandComponent* Neighbor = Info->XYtoComponentMap.FindRef(ComponentBase + FIntPoint(ComponentX, ComponentY));
			if (Neighbor)
			{
				// Data array for upscaling case
				TArray<FColor> CompHeightData;
				...
				FCyLandComponentDataInterface DataInterface(Neighbor, InLOD);
...
void FCyLandStaticLightingMesh::GetStaticLightingVertex(int32 VertexIndex, FStaticLightingVertex& OutVertex) const
{
	...
	// Copied from FCyLandComponentDataInterface::GetWorldPositionTangents to fix bad lighting when rotated
	...
	OutVertex.WorldPosition = LocalToWorld.TransformPosition(FVector(LocalX, LocalY, CyLandDataAccess::GetLocalHeight(Height)));
...
void FCyLandStaticLightingMesh::GetTriangle(int32 TriangleIndex,FStaticLightingVertex& OutV0,FStaticLightingVertex& OutV1,FStaticLightingVertex& OutV2) const
{
	int32 I0, I1, I2;
...
void FCyLandStaticLightingMesh::GetTriangleIndices(int32 TriangleIndex,int32& OutI0,int32& OutI1,int32& OutI2) const
{
...
FLightRayIntersection FCyLandStaticLightingMesh::IntersectLightRay(const FVector& Start,const FVector& End,bool bFindNearestIntersection) const
{
	// Intersect the light ray with the terrain component.
	FHitResult Result(1.0f);

	FHitResult NewHitInfo;
	FCollisionQueryParams NewTraceParams(SCENE_QUERY_STAT(FCyLandStaticLightingMesh_IntersectLightRay), true );
	
	const bool bIntersects = CyLandComponent->LineTraceComponent( Result, Start, End, NewTraceParams );
...
void UCyLandComponent::GetStaticLightingInfo(FStaticLightingPrimitiveInfo& OutPrimitiveInfo,const TArray<ULightComponent*>& InRelevantLights,const FLightingBuildOptions& Options)
{
	if( HasStaticLighting() && GetCyLandInfo() != nullptr)
	{
		const float LightMapRes = StaticLightingResolution > 0.f ? StaticLightingResolution : GetCyLandProxy()->StaticLightingResolution;
		const int32 LightingLOD = GetCyLandProxy()->StaticLightingLOD;
		...
		if (SizeX > 0 && SizeY > 0)
		{
			FCyLandStaticLightingMesh* StaticLightingMesh = new FCyLandStaticLightingMesh(this, InRelevantLights, PatchExpandCountX, PatchExpandCountY, LightMapRatio, LightingLOD);
			OutPrimitiveInfo.Meshes.Add(StaticLightingMesh);
			// Create a static lighting texture mapping
			OutPrimitiveInfo.Mappings.Add(new FCyLandStaticLightingTextureMapping(
				this,StaticLightingMesh,SizeX,SizeY,true));
		}
	}
}

void UCyLandComponent::AddMapBuildDataGUIDs(TSet<FGuid>& InGUIDs) const
{
	InGUIDs.Add(MapBuildDataId);
}

bool UCyLandComponent::GetLightMapResolution( int32& Width, int32& Height ) const
{
	// Assuming DXT_1 compression at the moment...
	float LightMapRes = StaticLightingResolution > 0.f ? StaticLightingResolution : GetCyLandProxy()->StaticLightingResolution;
	int32 PatchExpandCountX = 1;
	int32 PatchExpandCountY = 1;
	int32 DesiredSize = 1;
	uint32 LightingLOD = GetCyLandProxy()->StaticLightingLOD;
...
int32 UCyLandComponent::GetStaticLightMapResolution() const
{
	int32 Width, Height;
...
void UCyLandComponent::GetLightAndShadowMapMemoryUsage( int32& LightMapMemoryUsage, int32& ShadowMapMemoryUsage ) const
{
	int32 Width, Height;
	GetLightMapResolution(Width, Height);
...
void UCyLandComponent::InvalidateLightingCacheDetailed(bool bInvalidateBuildEnqueuedLighting, bool bTranslationOnly)
{
	Modify();
