...
IMPLEMENT_HIT_PROXY(HCyLandSplineProxy, HHitProxy);
IMPLEMENT_HIT_PROXY(HCyLandSplineProxy_Segment, HCyLandSplineProxy);
IMPLEMENT_HIT_PROXY(HCyLandSplineProxy_ControlPoint, HCyLandSplineProxy);
IMPLEMENT_HIT_PROXY(HCyLandSplineProxy_Tangent, HCyLandSplineProxy);

#define LOCTEXT_NAMESPACE "CyLand.Splines"
...
/** Represents a UCyLandSplinesComponent to the scene manager. */
#if WITH_EDITOR
class FCyLandSplinesSceneProxy final : public FPrimitiveSceneProxy
{
private:
	...
	{
		UCyLandSplineControlPoint* Owner;
		TRefCountPtr<HHitProxy> HitProxy;
		FVector Location;
		TArray<FCyLandSplineInterpPoint> Points;
		float SpriteScale;
...
public:
	...
	~FCyLandSplinesSceneProxy()
	{
	}

	FCyLandSplinesSceneProxy(UCyLandSplinesComponent* Component):
		...
		bDrawFalloff(Component->bShowSplineEditorMesh)
	{
		...
		for (UCyLandSplineSegment* Segment : Component->Segments)
		{
		...
		for (UCyLandSplineControlPoint* ControlPoint : Component->ControlPoints)
		{
	...
	virtual HHitProxy* CreateHitProxies(UPrimitiveComponent* Component, TArray<TRefCountPtr<HHitProxy> >& OutHitProxies) override
	{
		...
		{
			Segment.HitProxy = new HCyLandSplineProxy_Segment(Segment.Owner);
			OutHitProxies.Add(Segment.HitProxy);
		}
		for (FControlPointProxy& ControlPoint : ControlPoints)
		{
			ControlPoint.HitProxy = new HCyLandSplineProxy_ControlPoint(ControlPoint.Owner);
		...
		for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ViewIndex++)
		{
			if (VisibilityMap & (1 << ViewIndex))
			{
				...
				for (const FSegmentProxy& Segment : Segments)
				{
					...
					FCyLandSplineInterpPoint OldPoint = Segment.Points[0];
					...
					{
						FCyLandSplineInterpPoint NewPoint = Segment.Points[i];
				...
				for (const FControlPointProxy& ControlPoint : ControlPoints)
				{
					...
					{
						FCyLandSplineInterpPoint NewPoint = ControlPoint.Points[0];
						NewPoint.Center = MyLocalToWorld.TransformPosition(NewPoint.Center);
						NewPoint.Left   = MyLocalToWorld.TransformPosition(NewPoint.Left);
						NewPoint.Right  = MyLocalToWorld.TransformPosition(NewPoint.Right);
						NewPoint.FalloffLeft  = MyLocalToWorld.TransformPosition(NewPoint.FalloffLeft);
					...
					{
						FCyLandSplineInterpPoint OldPoint = ControlPoint.Points.Last();
						//OldPoint.Left   = MyLocalToWorld.TransformPosition(OldPoint.Left);
						OldPoint.Right  = MyLocalToWorld.TransformPosition(OldPoint.Right);
						//OldPoint.FalloffLeft  = MyLocalToWorld.TransformPosition(OldPoint.FalloffLeft);
						OldPoint.FalloffRight = MyLocalToWorld.TransformPosition(OldPoint.FalloffRight);

						for (const FCyLandSplineInterpPoint& Point : ControlPoint.Points)
						{
							FCyLandSplineInterpPoint NewPoint = Point;
							NewPoint.Center = MyLocalToWorld.TransformPosition(NewPoint.Center);
							NewPoint.Left   = MyLocalToWorld.TransformPosition(NewPoint.Left);
							NewPoint.Right  = MyLocalToWorld.TransformPosition(NewPoint.Right);
							NewPoint.FalloffLeft  = MyLocalToWorld.TransformPosition(NewPoint.FalloffLeft);
...
UCyLandSplinesComponent::UCyLandSplinesComponent(const FObjectInitializer& ObjectInitializer)
...
TArray<USplineMeshComponent*> UCyLandSplinesComponent::GetSplineMeshComponents()
{
	TArray<USplineMeshComponent*> SplineMeshComponents;
#if WITH_EDITOR
	for (UCyLandSplineSegment* SplineSegment : Segments)
	{
		// local
		SplineMeshComponents.Append(SplineSegment->GetLocalMeshComponents());
		
		// foreign
		TMap<UCyLandSplinesComponent*, TArray<USplineMeshComponent*>> ForeignMeshComponentsMap = SplineSegment->GetForeignMeshComponents();
...
void UCyLandSplinesComponent::CheckSplinesValid()
{
#if DO_CHECK
	...
	for (UCyLandSplineControlPoint* ControlPoint : ControlPoints)
	{
		ensure(ControlPoint->GetOuterUCyLandSplinesComponent() == this);
		for (const FCyLandSplineConnection& Connection : ControlPoint->ConnectedSegments)
		{
			ensure(Connection.Segment->GetOuterUCyLandSplinesComponent() == this);
		}
	}
	for (UCyLandSplineSegment* Segment : Segments)
	{
		ensure(Segment->GetOuterUCyLandSplinesComponent() == this);
		for (const FCyLandSplineSegmentConnection& Connection : Segment->Connections)
		{
			ensure(Connection.ControlPoint->GetOuterUCyLandSplinesComponent() == this);
		}
	}
#endif
}

void UCyLandSplinesComponent::OnRegister()
{
	CheckSplinesValid();
...
#if WITH_EDITOR
FPrimitiveSceneProxy* UCyLandSplinesComponent::CreateSceneProxy()
{
	CheckSplinesValid();

	return new FCyLandSplinesSceneProxy(this);
}
#endif

FBoxSphereBounds UCyLandSplinesComponent::CalcBounds(const FTransform& LocalToWorld) const
{
...
bool UCyLandSplinesComponent::ModifySplines(bool bAlwaysMarkDirty /*= true*/)
{
...
FArchive& operator<<(FArchive& Ar, FCyForeignSplineSegmentData& Value)
{
...
FArchive& operator<<(FArchive& Ar, FCyForeignWorldSplineData& Value)
{
	if (!Ar.IsFilterEditorOnly())
	{
...
void UCyLandSplinesComponent::Serialize(FArchive& Ar)
{
#if WITH_EDITORONLY_DATA
	...
	if (Ar.IsCooking())
	{
		...
		for (const auto& ForeignWorldSplineDataPair : ForeignWorldSplineDataMap)
		{
			...
			{
				CookedForeignMeshComponents.Add((UMeshComponent *)ForeignControlPointData.MeshComponent);
			}
		...
		!Ar.IsFilterEditorOnly())
	{
		...
		if (Ar.CustomVer(FCyLandCustomVersion::GUID) < FCyLandCustomVersion::NewSplineCrossLevelMeshSerialization)
		{
			Ar << ForeignWorldSplineDataMap;
		}

		if (Ar.IsLoading() && Ar.CustomVer(FCyLandCustomVersion::GUID) < FCyLandCustomVersion::SplineForeignDataLazyObjectPtrFix)
		{
...
#if WITH_EDITOR
void UCyLandSplinesComponent::AutoFixMeshComponentErrors(UWorld* OtherWorld)
{
	UWorld* ThisOuterWorld = GetTypedOuter<UWorld>();

	TSoftObjectPtr<UWorld> OtherWorldSoftPtr = OtherWorld;
	UCyLandSplinesComponent* StreamingSplinesComponent = GetStreamingSplinesComponentForLevel(OtherWorld->PersistentLevel);
...
bool UCyLandSplinesComponent::IsUsingEditorMesh(const USplineMeshComponent* SplineMeshComponent) const
{
	return SplineMeshComponent->GetStaticMesh() == SplineEditorMesh && SplineMeshComponent->bHiddenInGame;
}

void UCyLandSplinesComponent::CheckForErrors()
{
	...
	TMap<UWorld*, FCyForeignWorldSplineData*> ForeignWorldSplineDataMapCache;
	...
	for (UCyLandSplineControlPoint* ControlPoint : ControlPoints)
	{
		...
		if (ForeignWorld && !OutdatedWorlds.Contains(ForeignWorld))
		{
			...
			{
				UCyLandSplinesComponent* StreamingSplinesComponent = GetStreamingSplinesComponentForLevel(ForeignWorld->PersistentLevel);
	...
	for (UWorld* OutdatedWorld : OutdatedWorlds)
	{
		...
		FMessageLog("MapCheck").Error()
			...
			FOnActionTokenExecuted::CreateUObject(this, &UCyLandSplinesComponent::AutoFixMeshComponentErrors, OutdatedWorld), true));
	}
	...
	for (auto& ForeignWorldSplineDataPair : ForeignWorldSplineDataMap)
	{
		...
		{
			const UCyLandSplineSegment* ForeignSplineSegment = ForeignSplineSegmentData.Identifier.Get();
			...
			if (!ForeignSplineSegment)
			{
				...
				FMessageLog("MapCheck").Error()
					...
					FOnActionTokenExecuted::CreateUObject(this, &UCyLandSplinesComponent::DestroyOrphanedForeignMeshComponents, ForeignWorld), true));
...
void UCyLandSplinesComponent::PostLoad()
{
	Super::PostLoad();
...
void UCyLandSplinesComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
...
void UCyLandSplinesComponent::PostEditUndo()
{
...
void UCyLandSplinesComponent::RebuildAllSplines(bool bUpdateCollision)
{
	for (UCyLandSplineControlPoint* ControlPoint : ControlPoints)
	{
		ControlPoint->UpdateSplinePoints(true, false);
...
void UCyLandSplinesComponent::ShowSplineEditorMesh(bool bShow)
{
...
bool FCyForeignWorldSplineData::IsEmpty()
{
	return ForeignControlPointData.Num() == 0 && ForeignSplineSegmentData.Num() == 0;
}

FCyForeignControlPointData* FCyForeignWorldSplineData::FindControlPoint(UCyLandSplineControlPoint* InIdentifer)
{
...
FCyForeignSplineSegmentData* FCyForeignWorldSplineData::FindSegmentData(UCyLandSplineSegment* InIdentifer)
{
...
UCyLandSplinesComponent* UCyLandSplinesComponent::GetStreamingSplinesComponentByLocation(const FVector& LocalLocation, bool bCreate /* = true*/)
{
	ACyLandProxy* OuterCyLand = Cast<ACyLandProxy>(GetOwner());
	if (OuterCyLand &&
		...
		OuterCyLand->GetCyLandGuid().IsValid())
	{
		FVector CyLandLocalLocation = GetComponentTransform().GetRelativeTransform(OuterCyLand->CyLandActorToWorld()).TransformPosition(LocalLocation);
		const int32 ComponentIndexX = (CyLandLocalLocation.X >= 0.0f) ? FMath::FloorToInt(CyLandLocalLocation.X / OuterCyLand->ComponentSizeQuads) : FMath::CeilToInt(CyLandLocalLocation.X / OuterCyLand->ComponentSizeQuads);
		const int32 ComponentIndexY = (CyLandLocalLocation.Y >= 0.0f) ? FMath::FloorToInt(CyLandLocalLocation.Y / OuterCyLand->ComponentSizeQuads) : FMath::CeilToInt(CyLandLocalLocation.Y / OuterCyLand->ComponentSizeQuads);
		UCyLandComponent* CyLandComponent = OuterCyLand->GetCyLandInfo()->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX, ComponentIndexY));
		if (CyLandComponent)
		{
			ACyLandProxy* ComponentCyLandProxy = CyLandComponent->GetCyLandProxy();
			if (!ComponentCyLandProxy->SplineComponent && bCreate)
			{
				ComponentCyLandProxy->Modify();
				ComponentCyLandProxy->SplineComponent = NewObject<UCyLandSplinesComponent>(ComponentCyLandProxy, NAME_None, RF_Transactional);
				ComponentCyLandProxy->SplineComponent->RelativeScale3D = RelativeScale3D;
				ComponentCyLandProxy->SplineComponent->AttachToComponent(ComponentCyLandProxy->GetRootComponent(), FAttachmentTransformRules::KeepRelativeTransform);
			}
			if (ComponentCyLandProxy->SplineComponent)
			{
				return ComponentCyLandProxy->SplineComponent;
			}
		}
	}
...
UCyLandSplinesComponent* UCyLandSplinesComponent::GetStreamingSplinesComponentForLevel(ULevel* Level, bool bCreate /* = true*/)
{
	ACyLandProxy* OuterCyLand = Cast<ACyLandProxy>(GetOwner());
	if (OuterCyLand)
	{
		UCyLandInfo* CyLandInfo = OuterCyLand->GetCyLandInfo();
		check(CyLandInfo);

		ACyLandProxy* Proxy = CyLandInfo->GetCyLandProxyForLevel(Level);
		if (Proxy)
		{
			if (!Proxy->SplineComponent && bCreate)
			{
				Proxy->Modify();
				Proxy->SplineComponent = NewObject<UCyLandSplinesComponent>(Proxy, NAME_None, RF_Transactional);
...
TArray<UCyLandSplinesComponent*> UCyLandSplinesComponent::GetAllStreamingSplinesComponents()
{
	ACyLandProxy* OuterCyLand = Cast<ACyLandProxy>(GetOwner());
	if (OuterCyLand &&
		// when copy/pasting this can get called with a null guid on the parent landscape
		...
		OuterCyLand->GetCyLandGuid().IsValid())
	{
		...
		if (CyLandInfo)
		{
			TArray<UCyLandSplinesComponent*> SplinesComponents;
			CyLandInfo->ForAllCyLandProxies([&SplinesComponents](ACyLandProxy* Proxy)
			{
...
void UCyLandSplinesComponent::UpdateModificationKey(UCyLandSplineSegment* Owner)
{
...
void UCyLandSplinesComponent::UpdateModificationKey(UCyLandSplineControlPoint* Owner)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	checkSlow(OwnerWorld != GetTypedOuter<UWorld>());
...
void UCyLandSplinesComponent::AddForeignMeshComponent(UCyLandSplineSegment* Owner, USplineMeshComponent* Component)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	...
	FCyForeignSplineSegmentData* ForeignSplineSegmentData = ForeignWorldSplineData.FindSegmentData(Owner);

	if (ForeignSplineSegmentData == nullptr)
	{
		int32 AddedIndex = ForeignWorldSplineData.ForeignSplineSegmentData.Add(FCyForeignSplineSegmentData());
...
void UCyLandSplinesComponent::RemoveForeignMeshComponent(UCyLandSplineSegment* Owner, USplineMeshComponent* Component)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	...
	if (ForeignWorldSplineData)
	{
		FCyForeignSplineSegmentData* SegmentData = ForeignWorldSplineData->FindSegmentData(Owner);
...
void UCyLandSplinesComponent::RemoveAllForeignMeshComponents(UCyLandSplineSegment* Owner)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	checkSlow(OwnerWorld != GetTypedOuter<UWorld>());
...
void UCyLandSplinesComponent::AddForeignMeshComponent(UCyLandSplineControlPoint* Owner, UCyControlPointMeshComponent* Component)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	...
	int32 AddedIndex = ForeignWorldSplineData.ForeignControlPointData.Add(FCyForeignControlPointData());
...
void UCyLandSplinesComponent::RemoveForeignMeshComponent(UCyLandSplineControlPoint* Owner, UCyControlPointMeshComponent* Component)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
...
void UCyLandSplinesComponent::DestroyOrphanedForeignMeshComponents(UWorld* OwnerWorld)
{
	auto* ForeignWorldSplineData = ForeignWorldSplineDataMap.Find(OwnerWorld);
	...
	if (ForeignWorldSplineData)
	{
		...
		{
			FCyForeignSplineSegmentData& SegmentData = ForeignWorldSplineData->ForeignSplineSegmentData[i];
...
UCyControlPointMeshComponent* UCyLandSplinesComponent::GetForeignMeshComponent(UCyLandSplineControlPoint* Owner)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	checkSlow(OwnerWorld != GetTypedOuter<UWorld>());

	FCyForeignWorldSplineData* ForeignWorldSplineData = ForeignWorldSplineDataMap.Find(OwnerWorld);
	if (ForeignWorldSplineData)
	{
		FCyForeignControlPointData* ForeignControlPointData = ForeignWorldSplineData->FindControlPoint(Owner);
...
TArray<USplineMeshComponent*> UCyLandSplinesComponent::GetForeignMeshComponents(UCyLandSplineSegment* Owner)
{
	UWorld* OwnerWorld = Owner->GetTypedOuter<UWorld>();
	checkSlow(OwnerWorld != GetTypedOuter<UWorld>());
...
UObject* UCyLandSplinesComponent::GetOwnerForMeshComponent(const UMeshComponent* SplineMeshComponent)
{
...
UCyControlPointMeshComponent::UCyControlPointMeshComponent(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
...
UCyLandSplineControlPoint::UCyLandSplineControlPoint(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	Width = 1000;
	SideFalloff = 1000;
...
void UCyLandSplineControlPoint::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
...
#if WITH_EDITORONLY_DATA
	if (Ar.IsLoading() && Ar.CustomVer(FCyLandCustomVersion::GUID) < FCyLandCustomVersion::AddingBodyInstanceToSplinesElements)
	{
...
void UCyLandSplineControlPoint::PostLoad()
{
	Super::PostLoad();
	...
	if (GIsEditor)
	{
		...
		{
			UCyLandSplinesComponent* OuterSplines = GetOuterUCyLandSplinesComponent();
	...
	if (GIsEditor && GetLinkerCustomVersion(FCyLandCustomVersion::GUID) < FCyLandCustomVersion::AddingBodyInstanceToSplinesElements)
	{
...
FCyLandSplineSegmentConnection& FCyLandSplineConnection::GetNearConnection() const
{
	return Segment->Connections[End];
}

FCyLandSplineSegmentConnection& FCyLandSplineConnection::GetFarConnection() const
{
	return Segment->Connections[1 - End];
}

#if WITH_EDITOR
FName UCyLandSplineControlPoint::GetBestConnectionTo(FVector Destination) const
{
...
void UCyLandSplineControlPoint::GetConnectionLocalLocationAndRotation(FName SocketName, OUT FVector& OutLocation, OUT FRotator& OutRotation) const
{
...
void UCyLandSplineControlPoint::GetConnectionLocationAndRotation(FName SocketName, OUT FVector& OutLocation, OUT FRotator& OutRotation) const
{
...
void UCyLandSplineControlPoint::SetSplineSelected(bool bInSelected)
{
	bSelected = bInSelected;
	GetOuterUCyLandSplinesComponent()->MarkRenderStateDirty();
	...
	for (auto& ForeignMeshComponentsPair : ForeignMeshComponentsMap)
	{
		UCyLandSplinesComponent* MeshComponentOuterSplines = ForeignMeshComponentsPair.Key;
...
void UCyLandSplineControlPoint::AutoCalcRotation()
{
	Modify();

	FRotator Delta = FRotator::ZeroRotator;

	for (const FCyLandSplineConnection& Connection : ConnectedSegments)
	{
...
void UCyLandSplineControlPoint::AutoFlipTangents()
{
	for (const FCyLandSplineConnection& Connection : ConnectedSegments)
	{
...
void UCyLandSplineControlPoint::AutoSetConnections(bool bIncludingValid)
{
	for (const FCyLandSplineConnection& Connection : ConnectedSegments)
	{
		FCyLandSplineSegmentConnection& NearConnection = Connection.GetNearConnection();
		if (bIncludingValid ||
			(Mesh != nullptr && Mesh->FindSocket(NearConnection.SocketName) == nullptr) ||
			(Mesh == nullptr && NearConnection.SocketName != NAME_None))
		{
			FCyLandSplineSegmentConnection& FarConnection = Connection.GetFarConnection();
			FVector EndLocation; FRotator EndRotation;
...
#if WITH_EDITOR
TMap<UCyLandSplinesComponent*, UCyControlPointMeshComponent*> UCyLandSplineControlPoint::GetForeignMeshComponents()
{
	TMap<UCyLandSplinesComponent*, UCyControlPointMeshComponent*> ForeignMeshComponentsMap;

	UCyLandSplinesComponent* OuterSplines = GetOuterUCyLandSplinesComponent();
	TArray<UCyLandSplinesComponent*> SplineComponents = OuterSplines->GetAllStreamingSplinesComponents();

	for (UCyLandSplinesComponent* SplineComponent : SplineComponents)
	{
...
void UCyLandSplineControlPoint::UpdateSplinePoints(bool bUpdateCollision, bool bUpdateAttachedSegments)
{
	Modify();
	...
	UCyControlPointMeshComponent* MeshComponent = LocalMeshComponent;
	UCyLandSplinesComponent* MeshComponentOuterSplines = OuterSplines;
	...
	if (Mesh != nullptr)
	{
		...
		if (MeshComponent == nullptr)
		{
			...
			MeshComponent = NewObject<UCyControlPointMeshComponent>(MeshComponentOuterActor, NAME_None, RF_Transactional | RF_TextExportTransient);
	...
	for (auto& ForeignMeshComponentsPair : ForeignMeshComponentsMap)
	{
		UCyLandSplinesComponent* ForeignMeshComponentOuterSplines = ForeignMeshComponentsPair.Key;
	...
	if (Mesh != nullptr)
	{
		...
		Points.Sort([&CPLocation](const FCyLandSplineInterpPoint& x, const FCyLandSplineInterpPoint& y){return (x.Center - CPLocation).Rotation().Yaw < (y.Center - CPLocation).Rotation().Yaw;});
	}
	else
	{
		Points.Reset(1);
	...
	// Points bounds
	for (const FCyLandSplineInterpPoint& Point : Points)
	{
...
void UCyLandSplineControlPoint::DeleteSplinePoints()
{
	Modify();

	UCyLandSplinesComponent* OuterSplines = CastChecked<UCyLandSplinesComponent>(GetOuter());
...
void UCyLandSplineControlPoint::PostEditUndo()
{
	bHackIsUndoingSplines = true;
	Super::PostEditUndo();
	bHackIsUndoingSplines = false;
...
void UCyLandSplineControlPoint::PostDuplicate(bool bDuplicateForPIE)
{
	if (!bDuplicateForPIE)
	{
...
void UCyLandSplineControlPoint::PostEditImport()
{
	Super::PostEditImport();

	GetOuterUCyLandSplinesComponent()->ControlPoints.AddUnique(this);
}

void UCyLandSplineControlPoint::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
...
UCyLandSplineSegment::UCyLandSplineSegment(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
...
void UCyLandSplineSegment::PostInitProperties()
{
...
void UCyLandSplineSegment::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	...
	{
		for (FCyLandSplineMeshEntry& MeshEntry : SplineMeshes)
		{
...
void UCyLandSplineSegment::PostLoad()
{
	Super::PostLoad();
...
void UCyLandSplineSegment::UpdateMeshCollisionProfile(USplineMeshComponent* MeshComponent)
{
#if WITH_EDITORONLY_DATA
	UCyLandSplinesComponent* OuterSplines = CastChecked<UCyLandSplinesComponent>(GetOuter());
...
/**  */
#if WITH_EDITOR
void UCyLandSplineSegment::SetSplineSelected(bool bInSelected)
{
	bSelected = bInSelected;
	GetOuterUCyLandSplinesComponent()->MarkRenderStateDirty();
...
void UCyLandSplineSegment::AutoFlipTangents()
{
	FVector StartLocation; FRotator StartRotation;
...
bool FCyLandSplineMeshEntry::IsValid() const
{
	return Mesh != nullptr && ForwardAxis != UpAxis && Scale.GetAbsMin() > KINDA_SMALL_NUMBER;
}

#if WITH_EDITOR
TMap<UCyLandSplinesComponent*, TArray<USplineMeshComponent*>> UCyLandSplineSegment::GetForeignMeshComponents()
{
	TMap<UCyLandSplinesComponent*, TArray<USplineMeshComponent*>> ForeignMeshComponentsMap;
...
TArray<USplineMeshComponent*> UCyLandSplineSegment::GetLocalMeshComponents() const
{
	return LocalMeshComponents;
}

void UCyLandSplineSegment::UpdateSplinePoints(bool bUpdateCollision)
{
	Modify();
	...
	CyLandSplineRaster::Pointify(SplineInfo, Points, NumPoints, StartFalloffFraction, EndFalloffFraction, StartWidth, EndWidth, StartSideFalloff, EndSideFalloff, StartRollDegrees, EndRollDegrees);
	...
	TArray<const FCyLandSplineMeshEntry*> UsableMeshes;
	UsableMeshes.Reserve(SplineMeshes.Num());
	for (const FCyLandSplineMeshEntry& MeshEntry : SplineMeshes)
	{
	...
	FCyLandSplineMeshEntry SplineEditorMeshEntry;
	...
	if (SplineLength > 0 && (StartWidth > 0 || EndWidth > 0) && UsableMeshes.Num() > 0)
	{
		...
		{
			const float T;
			const FCyLandSplineMeshEntry* const MeshEntry;

			FMeshSettings(float InT, const FCyLandSplineMeshEntry* const InMeshEntry) :
		...
		while (T < 1.0f && iMesh < 20) // Max 20 meshes per spline segment
		{
			...
			const FCyLandSplineMeshEntry* MeshEntry = UsableMeshes[Random.RandHelper(UsableMeshes.Num())];
		...
		for (int32 i = 0; i < MeshComponents.Num(); i++)
		{
			...
			const FCyLandSplineMeshEntry* MeshEntry = MeshSettings[i].MeshEntry;
	...
	else
	{
		...
		for (auto& ForeignMeshComponentsPair : ForeignMeshComponentsMap)
		{
			...
			{
				UCyLandSplinesComponent* MeshComponentOuterSplines = dynamic_cast<UCyLandSplinesComponent*>(MeshComponent->GetAttachParent());
...
void UCyLandSplineSegment::UpdateSplineEditorMesh()
{
	UCyLandSplinesComponent* OuterSplines = CastChecked<UCyLandSplinesComponent>(GetOuter());
...
void UCyLandSplineSegment::DeleteSplinePoints()
{
	Modify();
...
void UCyLandSplineSegment::FindNearest( const FVector& InLocation, float& t, FVector& OutLocation, FVector& OutTangent )
{
...
bool UCyLandSplineSegment::Modify(bool bAlwaysMarkDirty /*= true*/)
{
...
#if WITH_EDITOR
void UCyLandSplineSegment::PostEditUndo()
{
	bHackIsUndoingSplines = true;
	Super::PostEditUndo();
	bHackIsUndoingSplines = false;
...
void UCyLandSplineSegment::PostDuplicate(bool bDuplicateForPIE)
{
	if (!bDuplicateForPIE)
	{
...
void UCyLandSplineSegment::PostEditImport()
{
	Super::PostEditImport();

	GetOuterUCyLandSplinesComponent()->Segments.AddUnique(this);

	if (Connections[0].ControlPoint != nullptr)
	{
		Connections[0].ControlPoint->ConnectedSegments.AddUnique(FCyLandSplineConnection(this, 0));
		Connections[1].ControlPoint->ConnectedSegments.AddUnique(FCyLandSplineConnection(this, 1));
	}
}

void UCyLandSplineSegment::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
