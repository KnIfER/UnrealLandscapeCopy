...
/*=============================================================================
CyLandEditInterface.cpp: CyLand editing interface
=============================================================================*/
...
//
// FCyLandEditDataInterface
//
FCyLandEditDataInterface::FCyLandEditDataInterface(UCyLandInfo* InCyLandInfo)
{
	if (InCyLandInfo)
	{
		CyLandInfo			= InCyLandInfo;
		ComponentSizeQuads		= InCyLandInfo->ComponentSizeQuads;
		SubsectionSizeQuads		= InCyLandInfo->SubsectionSizeQuads;
		ComponentNumSubsections = InCyLandInfo->ComponentNumSubsections;
		DrawScale				= InCyLandInfo->DrawScale;
	}
}

FCyLandTextureDataInterface::~FCyLandTextureDataInterface()
{
	Flush();
}

void FCyLandTextureDataInterface::Flush()
{
	bool bNeedToWaitForUpdate = false;

	// Update all textures
	for( TMap<UTexture2D*, FCyLandTextureDataInfo*>::TIterator It(TextureDataMap); It;  ++It )
	{
	...
	// delete all the FCyLandTextureDataInfo allocations
	for( TMap<UTexture2D*, FCyLandTextureDataInfo*>::TIterator It(TextureDataMap); It;  ++It )
	{
		delete It.Value();	// FCyLandTextureDataInfo destructors will unlock any texture data
	}
...
void ACyLand::CalcComponentIndicesOverlap(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const int32 ComponentSizeQuads, 
...
void ACyLand::CalcComponentIndicesNoOverlap(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const int32 ComponentSizeQuads, 
									  int32& ComponentIndexX1, int32& ComponentIndexY1, int32& ComponentIndexX2, int32& ComponentIndexY2)
{
	// Find component range for this block of data
...
bool FCyLandEditDataInterface::GetComponentsInRegion(int32 X1, int32 Y1, int32 X2, int32 Y2, TSet<UCyLandComponent*>* OutComponents /*= nullptr*/)
{
	if (ComponentSizeQuads <= 0 || !CyLandInfo)
	{
		return false;
	}
	// Find component range for this block of data
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		...
		{
			UCyLandComponent* Component = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX, ComponentIndexY));
			if (Component)
			{
				bNotLocked = bNotLocked && (!FLevelUtils::IsLevelLocked(Component->GetCyLandProxy()->GetLevel())) && FLevelUtils::IsLevelVisible(Component->GetCyLandProxy()->GetLevel());
				if (OutComponents)
				{
...
DEFINE_LOG_CATEGORY_STATIC(LogCyLandEditorInterface, Warning, All);
void FCyLandEditDataInterface::SetHeightData(int32 X1, int32 Y1, int32 X2, int32 Y2, const uint16* InData, int32 InStride, bool InCalcNormals, const uint16* InNormalData, bool InCreateComponents, UTexture2D* InHeightmap, UTexture2D* InXYOffsetmapTexture,
											   bool InUpdateBounds, bool InUpdateCollision, bool InGenerateMips)
{
	UE_LOG(LogCyLandEditorInterface, Warning, TEXT("Setting HeightData"));
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			UCyLandComponent* Component = CyLandInfo->XYtoComponentMap.FindRef(ComponentKey);
			...
			FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(Heightmap);
			FColor* HeightmapTextureData = (FColor*)TexDataInfo->GetMipData(0);

			FColor* XYOffsetMipData = nullptr;
			if (XYOffsetmapTexture)
			{
				FCyLandTextureDataInfo* XYTexDataInfo = GetTextureDataInfo(XYOffsetmapTexture);
			...
			for (int32 SubIndexY = SubIndexY1; SubIndexY <= SubIndexY2; SubIndexY++)
			{
				for (int32 SubIndexX = SubIndexX1; SubIndexX <= SubIndexX2; SubIndexX++)
				{
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						...
						{
							int32 CyLandX = SubIndexX*SubsectionSizeQuads + ComponentIndexX*ComponentSizeQuads + SubX;
							int32 CyLandY = SubIndexY*SubsectionSizeQuads + ComponentIndexY*ComponentSizeQuads + SubY;
							checkSlow(CyLandX >= X1 && CyLandX <= X2);
							checkSlow(CyLandY >= Y1 && CyLandY <= Y2);

							// Find the input data corresponding to this vertex
							int32 DataIndex = (CyLandX - X1) + InStride * (CyLandY - Y1);
							...
							if (VertexNormals && CyLandX > X1 && CyLandX < X2 && CyLandY > Y1 && CyLandY < Y2)
							{
								const int32 NormalDataIndex = (CyLandX - X1) + NumVertsX * (CyLandY - Y1);
			...
			if (InUpdateBounds)
			{
				float MinLocalZ = CyLandDataAccess::GetLocalHeight(MinHeight);
				float MaxLocalZ = CyLandDataAccess::GetLocalHeight(MaxHeight);
...
//
void FCyLandEditDataInterface::RecalculateNormals()
{
	...
	{
		UCyLandComponent* Component = It.Value();
		...
		FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(Component->GetHeightmap(true));
		FColor* HeightmapTextureData = (FColor*)TexDataInfo->GetMipData(0);
...
void FCyLandEditDataInterface::GetHeightDataTemplFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData, UTexture2D* InHeightmap, TStoreData* NormalData /*= NULL*/)
{
	if (!CyLandInfo) return;
	int32 ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2;
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{		
			...
			FCyLandTextureDataInfo* TexDataInfo = NULL;
				...
				for( int32 SubIndexX=SubIndexX1;SubIndexX<=SubIndexX2;SubIndexX++ )
				{
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							StoreData.Store(CyLandX, CyLandY, Height);
							if (NormalData)
							{
								uint16 Normals = (((uint16)TexData.B) << 8) | TexData.A;
								NormalData->Store(CyLandX, CyLandY, Normals);
							}
						}
					}
				}
...
void FCyLandEditDataInterface::CalcMissingValues(const int32& X1, const int32& X2, const int32& Y1, const int32& Y2, 
					...
					for( int32 SubIndexX=SubIndexX1;SubIndexX<=SubIndexX2;SubIndexX++ )
					{
						...
						for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
						{
							for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
							{
								...
								if (ExistLeft)
								{
									Value[0] = TData(StoreData.Load(ComponentIndexX*ComponentSizeQuads, CyLandY));
									Dist[0] = CyLandX - (ComponentIndexX*ComponentSizeQuads);
									Exist[0] = true;
								}
								else if ( BorderX1 != INT_MAX )
								{
									int32 BorderIdxX = (BorderX1+1)*ComponentSizeQuads;
									Value[0] = TData(StoreData.Load(BorderIdxX, CyLandY));
									Dist[0] = CyLandX - (BorderIdxX-1);
									Exist[0] = true;
								}
								else 
								{
									...
									{
										int32 Dist1 = CyLandY - (ComponentIndexY*ComponentSizeQuads);
										int32 Dist2 = ((ComponentIndexY+1)*ComponentSizeQuads) - CyLandY;
								...
								if ( BorderX2 != INT_MIN )
								{
									...
									Value[1] = TData(StoreData.Load(BorderIdxX, CyLandY));
									Dist[1] = (BorderIdxX+1) - CyLandX;
									Exist[1] = true;
								}
								else 
								{
									if ((CornerSet & 1 << 1) && (CornerSet & 1 << 3))
									{
										...
										Dist[1] = (ComponentIndexX+1)*ComponentSizeQuads - CyLandX;
										Exist[1] = true;
									}
								}
								...
								if (ExistUp)
								{
									Value[2] = TData(StoreData.Load(CyLandX, ComponentIndexY*ComponentSizeQuads));
									Dist[2] = CyLandY - (ComponentIndexY*ComponentSizeQuads);
									Exist[2] = true;
								}
								else if ( BorderY1[ComponentIndexXX] != INT_MAX )
								{
									int32 BorderIdxY = (BorderY1[ComponentIndexXX]+1)*ComponentSizeQuads;
									Value[2] = TData(StoreData.Load(CyLandX, BorderIdxY));
									Dist[2] = CyLandY - BorderIdxY;
									Exist[2] = true;
								}
								else 
								{
									...
									{
										int32 Dist1 = CyLandX - (ComponentIndexX*ComponentSizeQuads);
										int32 Dist2 = (ComponentIndexX+1)*ComponentSizeQuads - CyLandX;
								...
								if ( BorderY2[ComponentIndexXX] != INT_MIN )
								{
									...
									Value[3] = TData(StoreData.Load(CyLandX, BorderIdxY));
									Dist[3] = BorderIdxY - CyLandY;
									Exist[3] = true;
								}
								else 
								{
									if ((CornerSet & 1 << 2) && (CornerSet & 1 << 3))
									{
										...
										Dist[3] = (ComponentIndexY+1)*ComponentSizeQuads - CyLandY;
										Exist[3] = true;
									}
								}
								...
								StoreData.Store(CyLandX, CyLandY, FinalValue);
							}
						}
					}
				}
...
uint16 FCyLandEditDataInterface::GetHeightMapData(const UCyLandComponent* Component, int32 TexU, int32 TexV, FColor* TextureData /*= NULL*/)
{
	check(Component);
	if (!TextureData)
	{
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetHeightDataTempl(int32& ValidX1, int32& ValidY1, int32& ValidX2, int32& ValidY2, TStoreData& StoreData)
{
	// Copy variables
	...
	// Neighbor Components
	UCyLandComponent* BorderComponent[4] = {0, 0, 0, 0};
	UCyLandComponent* CornerComponent[4] = {0, 0, 0, 0};
	bool NoBorderX1 = false, NoBorderX2 = false;
	TArray<bool> NoBorderY1, NoBorderY2, ComponentDataExist;
	TArray<UCyLandComponent*> BorderComponentY1, BorderComponentY2;
	...
	FCyLandTextureDataInfo* NeighborTexDataInfo[4] = {0, 0, 0, 0};
		...
		for( int32 ComponentIndexX=ComponentIndexX1;ComponentIndexX<=ComponentIndexX2;ComponentIndexX++ )
		{
			...
			else
			{
				...
				if ( bShouldSearchX || (!NoBorderX1 && !BorderComponent[0]))
				{
					...
					for (int32 X = ComponentIndexX-1; X >= ComponentIndexX1; X--)
					{
						BorderComponent[0] = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(X,ComponentIndexY));
				...
				if ( bShouldSearchX || (!NoBorderX2 && !BorderComponent[1]))
				{
					...
					for (int32 X = ComponentIndexX+1; X <= ComponentIndexX2; X++)
					{
						BorderComponent[1] = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(X,ComponentIndexY));
				...
				if ( bShouldSearchY || (!NoBorderY1[ComponentIndexXX] && !BorderComponentY1[ComponentIndexXX]))
				{
					...
					for (int32 Y = ComponentIndexY-1; Y >= ComponentIndexY1; Y--)
					{
						BorderComponentY1[ComponentIndexXX] = BorderComponent[2] = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX,Y));
				...
				if ( bShouldSearchY || (!NoBorderY2[ComponentIndexXX] && !BorderComponentY2[ComponentIndexXX]))
				{
					...
					for (int32 Y = ComponentIndexY+1; Y <= ComponentIndexY2; Y++)
					{
						BorderComponentY2[ComponentIndexXX] = BorderComponent[3] = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX,Y));
				...
				CornerComponent[0] = ComponentIndexX >= ComponentIndexX1 && ComponentIndexY >= ComponentIndexY1 ?
					CyLandInfo->XYtoComponentMap.FindRef(FIntPoint((ComponentIndexX-1),(ComponentIndexY-1))) : NULL;
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							else
							{
								...
								if (ExistLeft)
								{
									Value[0] = StoreData.Load( ComponentIndexX*ComponentSizeQuads, CyLandY);
									Dist[0] = CyLandX - (ComponentIndexX*ComponentSizeQuads);
									Exist[0] = true;
								}
								else if (BorderComponent[0])
								{
									...
									Dist[0] = CyLandX - (BorderComponent[0]->GetSectionBase().X + ComponentSizeQuads);
									Exist[0] = true;
								}
								else 
								{
								...
								if (BorderComponent[1])
								{
									...
									Dist[1] = (BorderComponent[1]->GetSectionBase().X) - CyLandX;
									Exist[1] = true;
								}
								else
								{
								...
								if (ExistUp)
								{
									Value[2] = StoreData.Load( CyLandX, ComponentIndexY*ComponentSizeQuads);
									Dist[2] = CyLandY - (ComponentIndexY*ComponentSizeQuads);
									Exist[2] = true;
								}
								else if (BorderComponent[2])
								{
									...
									Dist[2] = CyLandY - (BorderComponent[2]->GetSectionBase().Y + ComponentSizeQuads);
									Exist[2] = true;
								}
								else
								{
								...
								if (BorderComponent[3])
								{
									...
									Dist[3] = (BorderComponent[3]->GetSectionBase().Y) - CyLandY;
									Exist[3] = true;
								}
								else
								{
								...
								StoreData.Store(CyLandX, CyLandY, FinalValue);
								//StoreData.StoreDefault(CyLandX, CyLandY);
							}
						}
					}
				}
...
namespace
{
	struct FArrayStoreData
	{
		...
		inline void Store(int32 CyLandX, int32 CyLandY, uint16 Height)
		{
			Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ] = Height;
		}

		// for interpolation
		inline uint16 Load(int32 CyLandX, int32 CyLandY)
		{
			return Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ];
		}

		inline void StoreDefault(int32 CyLandX, int32 CyLandY)
		{
			Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ] = 0;
		}
	};
	...
	struct FSparseStoreData
	{
		...
		inline void Store(int32 CyLandX, int32 CyLandY, uint16 Height)
		{
			SparseData.Add(FIntPoint(CyLandX,CyLandY), Height);
		}

		inline uint16 Load(int32 CyLandX, int32 CyLandY)
		{
			return SparseData.FindRef(FIntPoint(CyLandX,CyLandY));
		}
...
void FCyLandEditDataInterface::GetHeightData(int32& X1, int32& Y1, int32& X2, int32& Y2, uint16* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetHeightDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, uint16* Data, int32 Stride, uint16* NormalData /*= NULL*/, UTexture2D* InHeightmap)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetHeightData(int32& X1, int32& Y1, int32& X2, int32& Y2, TMap<FIntPoint, uint16>& SparseData)
{
	FSparseStoreData SparseStoreData(SparseData);
	GetHeightDataTempl(X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetHeightDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, uint16>& SparseData, TMap<FIntPoint, uint16>* NormalData /*= NULL*/, UTexture2D* InHeightmap)
{
	FSparseStoreData SparseStoreData(SparseData);
	if (NormalData)
	{
...
void UCyLandComponent::DeleteLayer(UCyLandLayerInfoObject* LayerInfo, FCyLandEditDataInterface& CyLandEdit)
{
	UCyLandComponent* Component = this;
	...
	const int32 DeleteLayerIdx = Component->WeightmapLayerAllocations.IndexOfByPredicate(
		[LayerInfo](const FCyWeightmapLayerAllocationInfo& Allocation) { return Allocation.LayerInfo == LayerInfo; });
	...
	FCyWeightmapLayerAllocationInfo& DeleteLayerAllocation = Component->WeightmapLayerAllocations[DeleteLayerIdx];
	...
	{
		FCyWeightmapLayerAllocationInfo& Allocation = Component->WeightmapLayerAllocations[LayerIdx];
	...
	if (!bDeleteLayerIsNoWeightBlend)
	{
		...
		TArray<FCyLandTextureDataInfo*> TexDataInfos;
		for (int32 WeightmapIdx = 0; WeightmapIdx < Component->WeightmapTextures.Num(); WeightmapIdx++)
		{
			TexDataInfos.Add(CyLandEdit.GetTextureDataInfo(Component->WeightmapTextures[WeightmapIdx]));
		}
		...
		for (int32 Idx = 0; Idx < Component->WeightmapTextures.Num(); Idx++)
		{
			...
			FCyLandTextureDataInfo* WeightmapDataInfo = TexDataInfos[Idx];
	...
	FCyLandWeightmapUsage* Usage = Proxy->WeightmapUsageMap.Find(Component->WeightmapTextures[DeleteLayerAllocation.WeightmapTextureIndex]);
	...
	for (int32 WeightmapIdx = 0; WeightmapIdx < Component->WeightmapTextures.Num(); WeightmapIdx++)
	{
		CollisionWeightmapMipData.Add((FColor*)CyLandEdit.GetTextureDataInfo(Component->WeightmapTextures[WeightmapIdx])->GetMipData(Component->CollisionMipLevel));
	}
	...
	if (Component->SimpleCollisionMipLevel > Component->CollisionMipLevel)
	{
		for (int32 WeightmapIdx = 0; WeightmapIdx < Component->WeightmapTextures.Num(); WeightmapIdx++)
		{
			SimpleCollisionWeightmapMipData.Add((FColor*)CyLandEdit.GetTextureDataInfo(Component->WeightmapTextures[WeightmapIdx])->GetMipData(Component->SimpleCollisionMipLevel));
		}
	}
...
void FCyLandEditDataInterface::DeleteLayer(UCyLandLayerInfoObject* LayerInfo)
{
	if (!CyLandInfo)
	{
		return;
	...
	for (auto& XYComponentPair : CyLandInfo->XYtoComponentMap)
	{
		UCyLandComponent* Component = XYComponentPair.Value;
		Component->DeleteLayer(LayerInfo, *this);
	}

	// Flush dynamic data (e.g. grass)
	TSet<UCyLandComponent*> Components;
	Algo::Transform(CyLandInfo->XYtoComponentMap, Components, &TPair<FIntPoint, UCyLandComponent*>::Value);
	ACyLandProxy::InvalidateGeneratedComponentData(Components);
}

void UCyLandComponent::FillLayer(UCyLandLayerInfoObject* LayerInfo, FCyLandEditDataInterface& CyLandEdit)
{
	check(LayerInfo);
	...
	if (FillLayerIdx == INDEX_NONE && !bFillLayerIsNoWeightBlend)
	{
		FillLayerIdx = Component->WeightmapLayerAllocations.IndexOfByPredicate(
			[](const FCyWeightmapLayerAllocationInfo& Allocation) { return !Allocation.LayerInfo || !Allocation.LayerInfo->bNoWeightBlend; });
	...
	if (FillLayerIdx == INDEX_NONE)
	{
		...
		Component->WeightmapLayerAllocations.Add(FCyWeightmapLayerAllocationInfo(LayerInfo));
		Component->ReallocateWeightmaps(&CyLandEdit);
	}
	...
	// fill the layer
	{
		...
		FCyWeightmapLayerAllocationInfo& FillLayerAllocation = Component->WeightmapLayerAllocations[FillLayerIdx];
		uint8* LayerData = (uint8*)CyLandEdit.GetTextureDataInfo(Component->WeightmapTextures[FillLayerAllocation.WeightmapTextureIndex])->GetMipData(0);
	...
	if (bClearOtherWeightBlendLayers)
	{
		for (int32 LayerIdx = 0; LayerIdx < Component->WeightmapLayerAllocations.Num(); ++LayerIdx)
		{
			...
			FCyLandWeightmapUsage* Usage = Proxy->WeightmapUsageMap.Find(Component->WeightmapTextures[Allocation.WeightmapTextureIndex]);
			if (Usage) // can be null if WeightmapUsageMap hasn't been built yet
			{
		...
		Component->WeightmapLayerAllocations.RemoveAll(
			[](const FCyWeightmapLayerAllocationInfo& Allocation) { return Allocation.WeightmapTextureIndex == 255; });
		...
		for (int32 TextureIdx = 0; TextureIdx < Component->WeightmapTextures.Num(); ++TextureIdx)
		{
			if (!Component->WeightmapLayerAllocations.ContainsByPredicate(
				[TextureIdx](const FCyWeightmapLayerAllocationInfo& Allocation) { return Allocation.WeightmapTextureIndex == TextureIdx; }))
			{
...
void FCyLandEditDataInterface::FillLayer(UCyLandLayerInfoObject* LayerInfo)
{
	if (!CyLandInfo)
	{
		return;
...
void FCyLandEditDataInterface::FillEmptyLayers(UCyLandLayerInfoObject* LayerInfo)
{
	if (!CyLandInfo)
	{
		return;
...
void UCyLandComponent::ReplaceLayer(UCyLandLayerInfoObject* FromLayerInfo, UCyLandLayerInfoObject* ToLayerInfo, FCyLandEditDataInterface& CyLandEdit)
{
	...
	FCyWeightmapLayerAllocationInfo& FromLayerAllocation = WeightmapLayerAllocations[FromLayerIdx];
	...
	if (bMerging)
	{
		FCyWeightmapLayerAllocationInfo& ToLayerAllocation = WeightmapLayerAllocations[ToLayerIdx];

		// Lock data for all the weightmaps
		FCyLandTextureDataInfo* FromTexDataInfo = CyLandEdit.GetTextureDataInfo(WeightmapTextures[FromLayerAllocation.WeightmapTextureIndex]);
		FCyLandTextureDataInfo* ToTexDataInfo = CyLandEdit.GetTextureDataInfo(WeightmapTextures[ToLayerAllocation.WeightmapTextureIndex]);
		...
		if (!bCanRemoveLayerTexture)
		{
			...
			FCyLandTextureDataInfo* WeightmapDataInfo = FromTexDataInfo;
		...
		if (FromTexDataInfo != ToTexDataInfo)
		{
			...
			FCyLandTextureDataInfo* WeightmapDataInfo = ToTexDataInfo;
	...
	if (bMerging)
	{
		...
		// Mark the channel as unallocated, so we can reuse it later
		FCyLandWeightmapUsage* Usage = Proxy->WeightmapUsageMap.Find(WeightmapTextures[FromLayerAllocation.WeightmapTextureIndex]);
		//check(Usage);
		if (Usage)
		{
...
void FCyLandEditDataInterface::ReplaceLayer(UCyLandLayerInfoObject* FromLayerInfo, UCyLandLayerInfoObject* ToLayerInfo)
{
	if (!CyLandInfo) return;
	for( auto It = CyLandInfo->XYtoComponentMap.CreateIterator(); It; ++It )
	{
...
namespace
{
	...
	struct TArrayStoreData
	{
		...
		inline void Store(int32 CyLandX, int32 CyLandY, uint8 Weight) {}
		inline void Store(int32 CyLandX, int32 CyLandY, uint8 Weight, int32 LayerIdx) {}
		inline void Store(int32 CyLandX, int32 CyLandY, FVector2D Offset) {}
		inline TDataType Load(int32 CyLandX, int32 CyLandY) { return 0; }
		inline void PreInit(int32 InArraySize) { ArraySize = InArraySize; }
	};

	template<> void TArrayStoreData<uint8>::Store(int32 CyLandX, int32 CyLandY, uint8 Weight)
	{
		Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ] = Weight;
	}

	template<> uint8 TArrayStoreData<uint8>::Load(int32 CyLandX, int32 CyLandY)
	{
		return Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ];
	}

	template<> FVector2D TArrayStoreData<FVector2D>::Load(int32 CyLandX, int32 CyLandY)
	{
		return Data[(CyLandY - Y1) * Stride + (CyLandX - X1)];
	}

	template<> FVector TArrayStoreData<FVector>::Load(int32 CyLandX, int32 CyLandY)
	{
		return Data[(CyLandY - Y1) * Stride + (CyLandX - X1)];
	}

	template<> void TArrayStoreData<FVector2D>::Store(int32 CyLandX, int32 CyLandY, FVector2D Offset)
	{
		Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ] = Offset;
	}

	template<> void TArrayStoreData<FVector>::Store(int32 CyLandX, int32 CyLandY, FVector2D Offset)
	{
		Data[ (CyLandY-Y1) * Stride + (CyLandX-X1) ] = FVector(Offset.X, Offset.Y, 0.0f);
	}

	// Data items should be initialized with ArraySize
	template<> void TArrayStoreData<TArray<uint8>>::Store(int32 CyLandX, int32 CyLandY, uint8 Weight, int32 LayerIdx)
	{
		TArray<uint8>& Value = Data[ ((CyLandY-Y1) * Stride + (CyLandX-X1)) ];
	...
	template<> void TSparseStoreData<uint8>::Store(int32 CyLandX, int32 CyLandY, uint8 Weight)
	{
		SparseData.Add(FIntPoint(CyLandX,CyLandY), Weight);
	}

	template<> uint8 TSparseStoreData<uint8>::Load(int32 CyLandX, int32 CyLandY)
	{
		return SparseData.FindRef(FIntPoint(CyLandX,CyLandY));
	}

	template<> FVector2D TSparseStoreData<FVector2D>::Load(int32 CyLandX, int32 CyLandY)
	{
		return SparseData.FindRef(FIntPoint(CyLandX, CyLandY));
	}

	template<> FVector TSparseStoreData<FVector>::Load(int32 CyLandX, int32 CyLandY)
	{
		return SparseData.FindRef(FIntPoint(CyLandX, CyLandY));
	}

	template<> void TSparseStoreData<TArray<uint8>>::Store(int32 CyLandX, int32 CyLandY, uint8 Weight, int32 LayerIdx)
	{
		TArray<uint8>* Value = SparseData.Find(FIntPoint(CyLandX,CyLandY));
		if (Value)
		{
			(*Value)[LayerIdx] = Weight;
		}
		else
		{
			...
			SparseData.Add(FIntPoint(CyLandX, CyLandY), Values);
		}
	}

	template<> void TSparseStoreData<FVector2D>::Store(int32 CyLandX, int32 CyLandY, FVector2D Offset)
	{
		SparseData.Add(FIntPoint(CyLandX,CyLandY), Offset);
	}

	template<> void TSparseStoreData<FVector>::Store(int32 CyLandX, int32 CyLandY, FVector2D Offset)
	{
		// Preserve old Z value
		FVector* PrevValue = SparseData.Find(FIntPoint(CyLandX,CyLandY));
		if (PrevValue != NULL)
		{
		...
		{
			SparseData.Add(FIntPoint(CyLandX,CyLandY), FVector(Offset.X, Offset.Y, 0.0f));
		}
	}
};

bool DeleteLayerIfAllZero(UCyLandComponent* const Component, const uint8* const TexDataPtr, int32 TexSize, int32 LayerIdx)
{
	...
	FCyLandWeightmapUsage& Usage = Proxy->WeightmapUsageMap.FindChecked(Component->WeightmapTextures[DeleteLayerWeightmapTextureIndex]);
	...
	bool bCanRemoveLayerTexture = !Component->WeightmapLayerAllocations.ContainsByPredicate([DeleteLayerWeightmapTextureIndex](const FCyWeightmapLayerAllocationInfo& Allocation){ return Allocation.WeightmapTextureIndex == DeleteLayerWeightmapTextureIndex; });
	if (bCanRemoveLayerTexture)
	{
		Component->WeightmapTextures[DeleteLayerWeightmapTextureIndex]->MarkPackageDirty();
		Component->WeightmapTextures[DeleteLayerWeightmapTextureIndex]->ClearFlags(RF_Standalone);
		...
		{
			FCyWeightmapLayerAllocationInfo& Allocation = *It;
			if (Allocation.WeightmapTextureIndex > DeleteLayerWeightmapTextureIndex)
			{
				Allocation.WeightmapTextureIndex--;
			}
...
inline bool FCyLandEditDataInterface::IsWhitelisted(const UCyLandLayerInfoObject* const LayerInfo, const int32 ComponentIndexX, const int32 SubIndexX, const int32 SubX, const int32 ComponentIndexY, const int32 SubIndexY, const int32 SubY)
{
	// left / right
	if (SubIndexX == 0 && SubX == 0)
	{
		UCyLandComponent* EdgeComponent = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX - 1, ComponentIndexY));
	...
	{
		UCyLandComponent* EdgeComponent = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX, ComponentIndexY - 1));
		if (EdgeComponent && !EdgeComponent->LayerWhitelist.Contains(LayerInfo))
		{
			return false;
		}
	...
	{
		UCyLandComponent* CornerComponent = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX - 1, ComponentIndexY - 1));
...
inline TMap<const UCyLandLayerInfoObject*, uint32> FCyLandEditDataInterface::CountWeightBlendedLayerInfluence(const int32 ComponentIndexX, const int32 ComponentIndexY, TOptional<TArrayView<const uint8* const>> InOptionalLayerDataPtrs)
{
	...
	TMap<const UCyLandLayerInfoObject*, uint32> LayerInfluenceMap;

	UCyLandComponent* Component = CyLandInfo->XYtoComponentMap.FindChecked(FIntPoint(ComponentIndexX,ComponentIndexY));

	// used if InOptionalLayerDataPtrs is null
	TArray<FCyLandTextureDataInfo*, TInlineAllocator<2>> InternalTexDataInfos;
	...
	else
	{
		...
		for (int32 LayerIdx = 0; LayerIdx < Component->WeightmapLayerAllocations.Num(); LayerIdx++)
		{
			const FCyWeightmapLayerAllocationInfo& Allocation = Component->WeightmapLayerAllocations[LayerIdx];
...
const UCyLandLayerInfoObject* FCyLandEditDataInterface::ChooseReplacementLayer(const UCyLandLayerInfoObject* const LayerInfo, const int32 ComponentIndexX, const int32 SubIndexX, const int32 SubX, const int32 ComponentIndexY, const int32 SubIndexY, const int32 SubY, TMap<FIntPoint, TMap<const UCyLandLayerInfoObject*, uint32>>& LayerInfluenceCache, TArrayView<const uint8* const> LayerDataPtrs)
{
	const TMap<const UCyLandLayerInfoObject*, uint32>* LayerInfluenceMapCacheEntry = LayerInfluenceCache.Find(FIntPoint(ComponentIndexX, ComponentIndexY));
	...
	TMap<const UCyLandLayerInfoObject*, uint32, TInlineSetAllocator<8>> LayerInfluenceMap = *LayerInfluenceMapCacheEntry;
	...
	for (int32 Y = ComponentYStart; Y <= ComponentYEnd; ++Y)
	{
		for (int32 X = ComponentXStart; X <= ComponentXEnd; ++X)
		{
			...
			UCyLandComponent* Component = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(X, Y));
			if (!Component)
			{
			...
			const TMap<const UCyLandLayerInfoObject*, uint32>* OtherLayerInfluenceMapCacheEntry = LayerInfluenceCache.Find(FIntPoint(X, Y));
...
void FCyLandEditDataInterface::SetAlphaData(UCyLandLayerInfoObject* const LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const uint8* Data, int32 Stride, ECyLandLayerPaintingRestriction PaintingRestriction /*= None*/, bool bWeightAdjust /*= true*/, bool bTotalWeightAdjust /*= false*/)
{
	...
	TArray<FCyLandTextureDataInfo*, TInlineAllocator<2>> TexDataInfos;
	...
	TMap<FIntPoint, TMap<const UCyLandLayerInfoObject*, uint32>> LayerInfluenceCache;
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			if (PaintingRestriction == ECyLandLayerPaintingRestriction::UseComponentWhitelist && !Component->LayerWhitelist.Contains(LayerInfo))
			{
				continue;
			}
			...
			int32 UpdateLayerIdx = Component->WeightmapLayerAllocations.IndexOfByPredicate([LayerInfo](const FCyWeightmapLayerAllocationInfo& Allocation){ return Allocation.LayerInfo == LayerInfo; });

			// Need allocation for weightmap
			if (UpdateLayerIdx == INDEX_NONE)
			{
				const int32 LayerLimit = Component->GetCyLandProxy()->MaxPaintedLayersPerComponent;

				// if we can't allocate a layer, then there is nothing to paint
				if (PaintingRestriction == ECyLandLayerPaintingRestriction::ExistingOnly ||
					(PaintingRestriction == ECyLandLayerPaintingRestriction::UseMaxLayers && LayerLimit > 0 && Component->WeightmapLayerAllocations.Num() >= LayerLimit))
				{
					continue;
				}

				UpdateLayerIdx = Component->WeightmapLayerAllocations.Num();
				new (Component->WeightmapLayerAllocations) FCyWeightmapLayerAllocationInfo(LayerInfo);
			...
			for (int32 SubIndexY = SubIndexY1; SubIndexY <= SubIndexY2; SubIndexY++)
			{
				for (int32 SubIndexX = SubIndexX1; SubIndexX <= SubIndexX2; SubIndexX++)
				{
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						for (int32 SubX = SubX1; SubX <= SubX2; SubX++)
						{
							const int32 CyLandX = SubIndexX*SubsectionSizeQuads + ComponentIndexX*ComponentSizeQuads + SubX;
							const int32 CyLandY = SubIndexY*SubsectionSizeQuads + ComponentIndexY*ComponentSizeQuads + SubY;
							checkSlow( CyLandX >= X1 && CyLandX <= X2 );
							checkSlow( CyLandY >= Y1 && CyLandY <= Y2 );

							// Find the input data corresponding to this vertex
							const int32 DataIndex = (CyLandX-X1) + Stride * (CyLandY-Y1);
							...
							if (PaintingRestriction == ECyLandLayerPaintingRestriction::UseComponentWhitelist && NewWeight != 0)
							{
								...
								else
								{
									...
									if (OtherLayerWeightSum == 0 && NewWeight < 255)
									{
										if (NewWeight < CurrentWeight)
										{
											...
											const UCyLandLayerInfoObject* ReplacementLayer = ChooseReplacementLayer(LayerInfo, ComponentIndexX, SubIndexX, SubX, ComponentIndexY, SubIndexY, SubY, LayerInfluenceCache, LayerDataPtrs);
											if (ReplacementLayer)
											{
												const int32 ReplacementLayerIndex = Component->WeightmapLayerAllocations.IndexOfByPredicate([&](const FCyWeightmapLayerAllocationInfo& AllocationInfo) { return AllocationInfo.LayerInfo == ReplacementLayer; });
			...
			for (int32 WeightmapIdx = 0; WeightmapIdx < Component->WeightmapTextures.Num(); WeightmapIdx++)
			{
				...
				UCyLandComponent::UpdateWeightmapMips(ComponentNumSubsections, SubsectionSizeQuads, WeightmapTexture, WeightmapTextureMipData, ComponentX1, ComponentY1, ComponentX2, ComponentY2, TexDataInfos[WeightmapIdx]);
				WeightmapTextureMipData.Reset();
			}
...
void FCyLandEditDataInterface::SetAlphaData(const TSet<UCyLandLayerInfoObject*>& DirtyLayerInfos, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, const uint8* Data, int32 Stride, ECyLandLayerPaintingRestriction PaintingRestriction /*= None*/)
{
	...
	for (UCyLandLayerInfoObject* LayerInfo : DirtyLayerInfos)
	{
		// The Data[] array passed in is indexed by CyLandInfo->GetLayerInfoIndex(),
		// so if we're trying to write a layer which isn't in the CyLandInfo,
		// its data is either missing or written where another layer's data should be.
		// Either way it's *very bad*.
		check(CyLandInfo->GetLayerInfoIndex(LayerInfo) != INDEX_NONE);
	}

	if (Stride == 0)
	{
		Stride = (1+X2-X1) * CyLandInfo->Layers.Num();
	}
	...
	TArray<UCyLandLayerInfoObject*, TInlineAllocator<8>> NeedAllocationInfos;
	TArray<FCyLandTextureDataInfo*, TInlineAllocator<2>> TexDataInfos;
	...
	for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
	{
		for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
		{
			...
			for (UCyLandLayerInfoObject* LayerInfo : DirtyLayerInfos)
			{
				const bool bFound = Component->WeightmapLayerAllocations.ContainsByPredicate([LayerInfo](const FCyWeightmapLayerAllocationInfo& Allocation){ return Allocation.LayerInfo == LayerInfo; });
				if (!bFound)
				{
			...
			if (NeedAllocationInfos.Num() > 0)
			{
				...
				{
					if (PaintingRestriction == ECyLandLayerPaintingRestriction::ExistingOnly ||
						(PaintingRestriction == ECyLandLayerPaintingRestriction::UseMaxLayers &&
				...
				{
					Component->Modify();
					for (UCyLandLayerInfoObject* LayerInfoNeedingAllocation : NeedAllocationInfos)
					{
						if (PaintingRestriction == ECyLandLayerPaintingRestriction::UseMaxLayers &&
			...
			{
				FCyWeightmapLayerAllocationInfo& Allocation = Component->WeightmapLayerAllocations[LayerIdx];
				const int32 LayerDataIdx = CyLandInfo->GetLayerInfoIndex(Component->WeightmapLayerAllocations[LayerIdx].LayerInfo);
				...
				for (int32 SubIndexX = SubIndexX1; SubIndexX <= SubIndexX2; SubIndexX++)
				{
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						for (int32 SubX = SubX1; SubX <= SubX2; SubX++)
						{
							...
							// Find the input data corresponding to this vertex
							const int32 DataIndex = (CyLandY-Y1) * Stride + (CyLandX-X1) * CyLandInfo->Layers.Num();
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetWeightDataTemplFast(UCyLandLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData)
{
	int32 ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2;
	ACyLand::CalcComponentIndicesNoOverlap(X1, Y1, X2, Y2, ComponentSizeQuads, ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2);
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{		
			...
			TArray<FCyLandTextureDataInfo*> TexDataInfos; // added for whole weight case...
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							if (LayerInfo != NULL)
							{
								...
								StoreData.Store(CyLandX, CyLandY, Weight);
							}
							else // Whole weight map case...
							{
								StoreData.PreInit(CyLandInfo->Layers.Num());
								for( int32 LayerIdx=0;LayerIdx<Component->WeightmapLayerAllocations.Num();LayerIdx++ )
								{
									...
									{
										int32 LayerInfoIdx = CyLandInfo->GetLayerInfoIndex(Component->WeightmapLayerAllocations[LayerIdx].LayerInfo);
										if (LayerInfoIdx != INDEX_NONE)
										{
											StoreData.Store(CyLandX, CyLandY, Weight, LayerInfoIdx);
										}
									}
								}
							}
...
uint8 FCyLandEditDataInterface::GetWeightMapData(const UCyLandComponent* Component, UCyLandLayerInfoObject* LayerInfo, int32 TexU, int32 TexV, uint8 Offset /*= 0*/, UTexture2D* Texture /*= NULL*/, uint8* TextureData /*= NULL*/)
{
	check(Component);
		...
		if (LayerInfo != NULL)
		{
			for( int32 LayerIdx=0;LayerIdx<Component->WeightmapLayerAllocations.Num();LayerIdx++ )
			{
				if( Component->WeightmapLayerAllocations[LayerIdx].LayerInfo == LayerInfo )
				{
					...
					FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(Texture);
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetWeightDataTempl(UCyLandLayerInfoObject* LayerInfo, int32& ValidX1, int32& ValidY1, int32& ValidX2, int32& ValidY2, TStoreData& StoreData)
{
	// Copy variables
	int32 X1 = ValidX1, X2 = ValidX2, Y1 = ValidY1, Y2 = ValidY2;
	ValidX1 = INT_MAX; ValidX2 = INT_MIN; ValidY1 = INT_MAX; ValidY2 = INT_MIN;
...
void FCyLandEditDataInterface::GetWeightData(UCyLandLayerInfoObject* LayerInfo, int32& X1, int32& Y1, int32& X2, int32& Y2, uint8* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetWeightDataFast(UCyLandLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, uint8* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetWeightData(UCyLandLayerInfoObject* LayerInfo, int32& X1, int32& Y1, int32& X2, int32& Y2, TMap<FIntPoint, uint8>& SparseData)
{
	TSparseStoreData<uint8> SparseStoreData(SparseData);
	GetWeightDataTempl(LayerInfo, X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetWeightDataFast(UCyLandLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, uint8>& SparseData)
{
	TSparseStoreData<uint8> SparseStoreData(SparseData);
	GetWeightDataTemplFast(LayerInfo, X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetWeightDataFast(UCyLandLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TArray<uint8>* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetWeightDataFast(UCyLandLayerInfoObject* LayerInfo, const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, TArray<uint8>>& SparseData)
{
	TSparseStoreData<TArray<uint8>> SparseStoreData(SparseData);
	GetWeightDataTemplFast(LayerInfo, X1, Y1, X2, Y2, SparseStoreData);
}

FCyLandTextureDataInfo* FCyLandTextureDataInterface::GetTextureDataInfo(UTexture2D* Texture)
{
	FCyLandTextureDataInfo* Result = TextureDataMap.FindRef(Texture);
	if( !Result )
	{
		Result = TextureDataMap.Add(Texture, new FCyLandTextureDataInfo(Texture));
	}
	return Result;
}

void FCyLandTextureDataInterface::CopyTextureChannel(UTexture2D* Dest, int32 DestChannel, UTexture2D* Src, int32 SrcChannel)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Dest);
	FCyLandTextureDataInfo* SrcDataInfo = GetTextureDataInfo(Src);
...
void FCyLandTextureDataInterface::CopyTextureFromHeightmap(UTexture2D* Dest, int32 DestChannel, UCyLandComponent* Comp, int32 SrcChannel)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Dest);
	int32 MipSize = Dest->Source.GetSizeX();
	...
	for( int32 MipIdx=0;MipIdx<DestDataInfo->NumMips();MipIdx++ )
	{
		FCyLandComponentDataInterface DataInterface(Comp, MipIdx);
...
void FCyLandTextureDataInterface::CopyTextureFromWeightmap(UTexture2D* Dest, int32 DestChannel, UCyLandComponent* Comp, UCyLandLayerInfoObject* LayerInfo)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Dest);
	int32 MipSize = Dest->Source.GetSizeX();
	check(Dest->Source.GetSizeX() == Dest->Source.GetSizeY());
...
void FCyLandTextureDataInterface::ZeroTextureChannel(UTexture2D* Dest, int32 DestChannel)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Dest);
	int32 MipSize = Dest->Source.GetSizeX();
	check(Dest->Source.GetSizeX() == Dest->Source.GetSizeY());
...
void FCyLandTextureDataInterface::SetTextureValueTempl(UTexture2D* Dest, TData Value)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Dest);
	int32 MipSize = Dest->Source.GetSizeX();
	check(Dest->Source.GetSizeX() == Dest->Source.GetSizeY());
...
void FCyLandTextureDataInterface::ZeroTexture(UTexture2D* Dest)
{
	SetTextureValueTempl<uint8>(Dest, 0);
}

void FCyLandTextureDataInterface::SetTextureValue(UTexture2D* Dest, FColor Value)
{
	SetTextureValueTempl<FColor>(Dest, Value);
}

template<typename TData>
bool FCyLandTextureDataInterface::EqualTextureValueTempl(UTexture2D* Src, TData Value)
{
	FCyLandTextureDataInfo* DestDataInfo = GetTextureDataInfo(Src);
...
bool FCyLandTextureDataInterface::EqualTextureValue(UTexture2D* Src, FColor Value)
{
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetSelectDataTempl(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData)
{
	int32 ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2;
	ACyLand::CalcComponentIndicesNoOverlap(X1, Y1, X2, Y2, ComponentSizeQuads, ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2);
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							if( Component && SelectTextureData )
							{
								...
								StoreData.Store(CyLandX, CyLandY, TexData);
							}
							else
							{
								StoreData.Store(CyLandX, CyLandY, 0);
							}
...
void FCyLandEditDataInterface::GetSelectData(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, uint8>& SparseData)
{
	TSparseStoreData<uint8> SparseStoreData(SparseData);
	GetSelectDataTempl(X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetSelectData(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, uint8* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::SetSelectData(int32 X1, int32 Y1, int32 X2, int32 Y2, const uint8* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
	...
	for( int32 ComponentIndexY=ComponentIndexY1;ComponentIndexY<=ComponentIndexY2;ComponentIndexY++ )
	{
		...
		{	
			...
			else if (Component->EditToolRenderData.DataTexture == NULL)
			{
				...
				DataTexture = Component->GetCyLandProxy()->CreateCyLandTexture(WeightmapSize, WeightmapSize, TEXTUREGROUP_Terrain_Weightmap, TSF_G8);
				// Alloc dummy mips
				UCyLandComponent::CreateEmptyTextureMips(DataTexture, true);
				DataTexture->PostEditChange();

				//FlushRenderingCommands();
				ZeroTexture(DataTexture);
				FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(DataTexture);
				...
				UCyLandComponent::UpdateDataMips(ComponentNumSubsections, SubsectionSizeQuads, DataTexture, TextureMipData, 0, 0, MAX_int32, MAX_int32, TexDataInfo);
				...
				for( int32 SubIndexX=SubIndexX1;SubIndexX<=SubIndexX2;SubIndexX++ )
				{
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							// Find the input data corresponding to this vertex
							int32 DataIndex = (CyLandX-X1) + Stride * (CyLandY-Y1);
			...
			UCyLandComponent::UpdateDataMips(ComponentNumSubsections, SubsectionSizeQuads, DataTexture, TextureMipData, ComponentX1, ComponentY1, ComponentX2, ComponentY2, TexDataInfo);
		}
	}
}

template<typename T>
void FCyLandEditDataInterface::SetXYOffsetDataTempl(int32 X1, int32 Y1, int32 X2, int32 Y2, const T* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
		...
		{	
			...
			else
			{
				if (Component->XYOffsetmapTexture == NULL)
				{
					...
					// Construct Texture...
					int32 WeightmapSize = (Component->SubsectionSizeQuads+1) * Component->NumSubsections;
					XYOffsetTexture = Component->GetCyLandProxy()->CreateCyLandTexture(WeightmapSize, WeightmapSize, TEXTUREGROUP_Terrain_Weightmap, TSF_BGRA8);
					// Alloc dummy mips
					UCyLandComponent::CreateEmptyTextureMips(XYOffsetTexture, true);
					XYOffsetTexture->PostEditChange();

					//FlushRenderingCommands();
					SetTextureValue(XYOffsetTexture, DefaultValue);
					FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(XYOffsetTexture);
					...
					UCyLandComponent::UpdateWeightmapMips(ComponentNumSubsections, SubsectionSizeQuads, XYOffsetTexture, TextureMipData, 0, 0, MAX_int32, MAX_int32, TexDataInfo);
			...
			UCyLandComponent::UpdateWeightmapMips(ComponentNumSubsections, SubsectionSizeQuads, XYOffsetTexture, TextureMipData, ComponentX1, ComponentY1, ComponentX2, ComponentY2, TexDataInfo);
		}
	}
}

void FCyLandEditDataInterface::SetXYOffsetData(int32 X1, int32 Y1, int32 X2, int32 Y2, const FVector2D* Data, int32 Stride)
{
	SetXYOffsetDataTempl<FVector2D>(X1, Y1, X2, Y2, Data, Stride);
}

void FCyLandEditDataInterface::SetXYOffsetData(int32 X1, int32 Y1, int32 X2, int32 Y2, const FVector* Data, int32 Stride)
{
	SetXYOffsetDataTempl<FVector>(X1, Y1, X2, Y2, Data, Stride);
}

FVector2D FCyLandEditDataInterface::GetXYOffsetmapData(const UCyLandComponent* Component, int32 TexU, int32 TexV, FColor* TextureData/* = NULL*/)
{
	check(Component);
	if (!TextureData && Component->XYOffsetmapTexture)
	{
		FCyLandTextureDataInfo* TexDataInfo = GetTextureDataInfo(Component->XYOffsetmapTexture);
		TextureData = (FColor*)TexDataInfo->GetMipData(0);	
	}
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetXYOffsetDataTempl(int32& ValidX1, int32& ValidY1, int32& ValidX2, int32& ValidY2, TStoreData& StoreData)
{
	// Copy variables
	int32 X1 = ValidX1, X2 = ValidX2, Y1 = ValidY1, Y2 = ValidY2;
	ValidX1 = INT_MAX; ValidX2 = INT_MIN; ValidY1 = INT_MAX; ValidY2 = INT_MIN;
	...
	int32 XYOffsetSize = (CyLandInfo->SubsectionSizeQuads + 1) * CyLandInfo->ComponentNumSubsections;
					...
					for (int32 SubY = SubY1; SubY <= SubY2; SubY++)
					{
						for (int32 SubX = SubX1; SubX <= SubX2; SubX++)
						{
							...
							if (Component)
							{
								...
								StoreData.Store(CyLandX, CyLandY, XYOffset);
							}
							else
							{
								// Find the texture data corresponding to this vertex
								...
								if (ExistLeft)
								{
									Value[0] = FVector2D(StoreData.Load(ComponentIndexX*ComponentSizeQuads, CyLandY));
									Dist[0] = CyLandX - (ComponentIndexX*ComponentSizeQuads);
									Exist[0] = true;
								}
								else if (BorderComponent[0])
								...
								if (ExistUp)
								{
									Value[2] = FVector2D(StoreData.Load(CyLandX, ComponentIndexY*ComponentSizeQuads));
									Dist[2] = CyLandY - (ComponentIndexY*ComponentSizeQuads);
									Exist[2] = true;
								}
								else if (BorderComponent[2])
...
void FCyLandEditDataInterface::GetXYOffsetData(int32& X1, int32& Y1, int32& X2, int32& Y2, FVector2D* Data, int32 Stride)
{
	if (Stride == 0)
	{
		Stride = (1 + X2 - X1);
...
void FCyLandEditDataInterface::GetXYOffsetData(int32& X1, int32& Y1, int32& X2, int32& Y2, TMap<FIntPoint, FVector2D>& SparseData)
{
	TSparseStoreData<FVector2D> SparseStoreData(SparseData);
	GetXYOffsetDataTempl(X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetXYOffsetData(int32& X1, int32& Y1, int32& X2, int32& Y2, FVector* Data, int32 Stride)
{
	if (Stride == 0)
	{
		Stride = (1 + X2 - X1);
...
void FCyLandEditDataInterface::GetXYOffsetData(int32& X1, int32& Y1, int32& X2, int32& Y2, TMap<FIntPoint, FVector>& SparseData)
{
...
template<typename TStoreData>
void FCyLandEditDataInterface::GetXYOffsetDataTemplFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TStoreData& StoreData)
{
	int32 ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2;
	ACyLand::CalcComponentIndicesNoOverlap(X1, Y1, X2, Y2, ComponentSizeQuads, ComponentIndexX1, ComponentIndexY1, ComponentIndexX2, ComponentIndexY2);
					...
					for( int32 SubY=SubY1;SubY<=SubY2;SubY++ )
					{
						for( int32 SubX=SubX1;SubX<=SubX2;SubX++ )
						{
							...
							if( Component && OffsetTextureData )
							{
								...
								StoreData.Store(CyLandX, CyLandY, Value);
							}
							else
							{
								StoreData.Store(CyLandX, CyLandY, FVector2D(0.0f, 0.0f) );
							}
						}
					}
				}
...
void FCyLandEditDataInterface::GetXYOffsetDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, FVector2D* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetXYOffsetDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, FVector2D>& SparseData)
{
	TSparseStoreData<FVector2D> SparseStoreData(SparseData);
	GetXYOffsetDataTemplFast(X1, Y1, X2, Y2, SparseStoreData);
}

void FCyLandEditDataInterface::GetXYOffsetDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, FVector* Data, int32 Stride)
{
	if( Stride==0 )
	{
		Stride = (1+X2-X1);
...
void FCyLandEditDataInterface::GetXYOffsetDataFast(const int32 X1, const int32 Y1, const int32 X2, const int32 Y2, TMap<FIntPoint, FVector>& SparseData)
{
	TSparseStoreData<FVector> SparseStoreData(SparseData);
	GetXYOffsetDataTemplFast(X1, Y1, X2, Y2, SparseStoreData);
}
...
//
// FCyLandTextureDataInfo
//

FCyLandTextureDataInfo::FCyLandTextureDataInfo(UTexture2D* InTexture)
:	Texture(InTexture)
{
...
bool FCyLandTextureDataInfo::UpdateTextureData()
{
...
FCyLandTextureDataInfo::~FCyLandTextureDataInfo()
{
