...
namespace CyLandCollisionCookStats
{
	...
	{
		HeightfieldUsageStats.LogStats(AddStat, TEXT("CyLandCollision.Usage"), TEXT("Heightfield"));
		MeshUsageStats.LogStats(AddStat, TEXT("CyLandCollision.Usage"), TEXT("Mesh"));
	});
}
#endif

TMap<FGuid, UCyLandHeightfieldCollisionComponent::FPhysXHeightfieldRef* > GSharedHeightfieldRefs;
...
TMap<FGuid, UCyLandMeshCollisionComponent::FPhysXMeshRef* > GSharedMeshRefs;
...
ECollisionEnabled::Type UCyLandHeightfieldCollisionComponent::GetCollisionEnabled() const
{
...
ECollisionResponse UCyLandHeightfieldCollisionComponent::GetCollisionResponseToChannel(ECollisionChannel Channel) const
{
	ACyLandProxy* Proxy = GetCyLandProxy();
...
ECollisionChannel UCyLandHeightfieldCollisionComponent::GetCollisionObjectType() const
{
	ACyLandProxy* Proxy = GetCyLandProxy();
...
const FCollisionResponseContainer& UCyLandHeightfieldCollisionComponent::GetCollisionResponseToChannels() const
{
	ACyLandProxy* Proxy = GetCyLandProxy();
...
void UCyLandHeightfieldCollisionComponent::OnCreatePhysicsState()
{
	...
	if (!BodyInstance.IsValidBodyInstance())
	{
		...
		if (IsValidRef(HeightfieldRef))
		{
			...
			FTransform CyLandComponentTransform = GetComponentToWorld();
			FMatrix CyLandComponentMatrix = CyLandComponentTransform.ToMatrixWithScale();
			bool bIsMirrored = CyLandComponentMatrix.Determinant() < 0.f;
			if (!bIsMirrored)
			{
				// Unreal and PhysX have opposite handedness, so we need to translate the origin and rearrange the data
				CyLandComponentMatrix = FTranslationMatrix(FVector(CollisionSizeQuads*CollisionScale, 0, 0)) * CyLandComponentMatrix;
			}

			// Get the scale to give to PhysX
			FVector CyLandScale = CyLandComponentMatrix.ExtractScaling();

			// Reorder the axes
			FVector TerrainX = CyLandComponentMatrix.GetScaledAxis(EAxis::X);
			FVector TerrainY = CyLandComponentMatrix.GetScaledAxis(EAxis::Y);
			FVector TerrainZ = CyLandComponentMatrix.GetScaledAxis(EAxis::Z);
			CyLandComponentMatrix.SetAxis(0, TerrainX);
			CyLandComponentMatrix.SetAxis(2, TerrainY);
			CyLandComponentMatrix.SetAxis(1, TerrainZ);

			PxTransform PhysXCyLandComponentTransform = U2PTransform(FTransform(CyLandComponentMatrix));
			...
			// Create the geometry
			PxHeightFieldGeometry CyLandComponentGeom(HeightfieldRef->RBHeightfield, PxMeshGeometryFlag::eDOUBLE_SIDED, CyLandScale.Z * LANDSCAPE_ZSCALE, CyLandScale.Y * CollisionScale, CyLandScale.X * CollisionScale);

			if (CyLandComponentGeom.isValid())
			{
				// Creating both a sync and async actor, since this object is static

				// Create the sync scene actor
				PxRigidStatic* HeightFieldActorSync = GPhysXSDK->createRigidStatic(PhysXCyLandComponentTransform);
				PxShape* HeightFieldShapeSync = GPhysXSDK->createShape(CyLandComponentGeom, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
				...
				{
					PxHeightFieldGeometry CyLandComponentGeomSimple(HeightfieldRef->RBHeightfieldSimple, PxMeshGeometryFlags(), CyLandScale.Z * LANDSCAPE_ZSCALE, CyLandScale.Y * SimpleCollisionScale, CyLandScale.X * SimpleCollisionScale);
					check(CyLandComponentGeomSimple.isValid());
					PxShape* HeightFieldShapeSimpleSync = GPhysXSDK->createShape(CyLandComponentGeomSimple, HeightfieldRef->UsedPhysicalMaterialArray.GetData(), HeightfieldRef->UsedPhysicalMaterialArray.Num(), true);
				...
				{
					PxHeightFieldGeometry CyLandComponentGeomEd(HeightfieldRef->RBHeightfieldEd, PxMeshGeometryFlags(), CyLandScale.Z * LANDSCAPE_ZSCALE, CyLandScale.Y * CollisionScale, CyLandScale.X * CollisionScale);
					if (CyLandComponentGeomEd.isValid())
					{
						...
						PxShape* HeightFieldEdShapeSync = GPhysXSDK->createShape(CyLandComponentGeomEd, &PDefaultMat, 1, true);
...
void UCyLandHeightfieldCollisionComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
...
void UCyLandHeightfieldCollisionComponent::CreateCollisionObject()
{
#if WITH_PHYSX
...
#if WITH_EDITOR
void UCyLandHeightfieldCollisionComponent::SpeculativelyLoadAsyncDDCCollsionData()
{
#if WITH_PHYSX
...
#if WITH_PHYSX
TArray<PxHeightFieldSample> ConvertHeightfieldDataForPhysx(const UCyLandHeightfieldCollisionComponent* const Component, const int32 CollisionSizeVerts, const bool bIsMirrored, const uint16* Heights, const bool bUseDefMaterial, const uint8* DominantLayers, UPhysicalMaterial* const DefMaterial, TArray<UPhysicalMaterial*> &InOutMaterials)
{
		...
		for (int32 ColIndex = 0; ColIndex < CollisionSizeVerts; ColIndex++)
		{
				...
				ColIndex < CollisionSizeVerts - 1)
			{
				...
				if (!bUseDefMaterial && DominantLayers)
				{
					...
					{
						UCyLandLayerInfoObject* Layer = Component->ComponentLayerInfos[DominantLayerIdx];
						if (Layer == ACyLandProxy::VisibilityLayer)
						{
...
bool UCyLandHeightfieldCollisionComponent::CookCollisionData(const FName& Format, bool bUseDefMaterial, bool bCheckDDC, TArray<uint8>& OutCookedData, TArray<UPhysicalMaterial*>& InOutMaterials) const
{
#if WITH_PHYSX
	COOK_STAT(auto Timer = CyLandCollisionCookStats::HeightfieldUsageStats.TimeSyncWork());
	...
	if (bCheckDDC && HeightfieldGuid.IsValid())
	{
		...
		if (GetLinkerUE4Version() >= VER_UE4_LANDSCAPE_SERIALIZE_PHYSICS_MATERIALS)
		{
			...
			if (SpeculativeDDCRequest.IsValid() && DDCKey == SpeculativeDDCRequest->GetKey())
			{
				...
				COOK_STAT(auto WaitTimer = CyLandCollisionCookStats::HeightfieldUsageStats.TimeAsyncWait());
	...
	const FVector CyLandScale = Proxy->GetRootComponent()->RelativeScale3D;
	const bool bIsMirrored = (CyLandScale.X*CyLandScale.Y*CyLandScale.Z) < 0.f;
...
bool UCyLandMeshCollisionComponent::CookCollisionData(const FName& Format, bool bUseDefMaterial, bool bCheckDDC, TArray<uint8>& OutCookedData, TArray<UPhysicalMaterial*>& InOutMaterials) const
{
#if WITH_PHYSX
	COOK_STAT(auto Timer = CyLandCollisionCookStats::MeshUsageStats.TimeSyncWork());
	// we have 2 versions of collision objects
	const int32 CookedDataIndex = bUseDefMaterial ? 0 : 1;
	...
	if (bCheckDDC)
	{
		...
		if (GetLinkerUE4Version() >= VER_UE4_LANDSCAPE_SERIALIZE_PHYSICS_MATERIALS && MeshGuid.IsValid())
		{
			...
			{
				// If we have a DDC request in flight, just time the synchronous cycles used.
				COOK_STAT(auto WaitTimer = CyLandCollisionCookStats::MeshUsageStats.TimeAsyncWait());
				SpeculativeDDCRequest->WaitAsynchronousCompletion();
				bool bSuccess = SpeculativeDDCRequest->GetAsynchronousResults(OutCookedData);
				// World will clean up remaining reference
				SpeculativeDDCRequest.Reset();
	...
	for (int32 y = 0; y < CollisionSizeQuads; y++)
	{
		for (int32 x = 0; x < CollisionSizeQuads; x++)
		{
			...
			if (!bUseDefMaterial && DominantLayers)
			{
				...
				{
					UCyLandLayerInfoObject* Layer = ComponentLayerInfos[DominantLayerIdx];
					if (Layer == ACyLandProxy::VisibilityLayer)
					{
						// If it's a hole, override with the hole flag.
						bHole = true;
...
void UCyLandMeshCollisionComponent::CreateCollisionObject()
{
#if WITH_PHYSX	
	// If we have not created a heightfield yet - do it now.
...
void UCyLandMeshCollisionComponent::OnCreatePhysicsState()
{
	USceneComponent::OnCreatePhysicsState(); // route OnCreatePhysicsState, skip PrimitiveComponent implementation
	...
	if (!BodyInstance.IsValidBodyInstance())
	{
		...
		if (IsValidRef(MeshRef))
		{
			...
			{
				// Unreal and PhysX have opposite handedness, so we need to translate the origin and rearrange the data
				CyLandComponentMatrix = FTranslationMatrix(FVector(CollisionSizeQuads, 0, 0)) * CyLandComponentMatrix;
			}
			...
			PTriMeshGeom.scale.scale.x = CyLandScale.X * CollisionScale;
			PTriMeshGeom.scale.scale.y = CyLandScale.Y * CollisionScale;
			PTriMeshGeom.scale.scale.z = CyLandScale.Z;
			...
			if (PTriMeshGeom.isValid())
			{
				...
				// Create the sync scene actor
				PxRigidStatic* MeshActorSync = GPhysXSDK->createRigidStatic(PhysXCyLandComponentTransform);
				...
				if (!GetWorld()->IsGameWorld())
				{
					...
					PTriMeshGeomEd.scale.scale.x = CyLandScale.X * CollisionScale;
					PTriMeshGeomEd.scale.scale.y = CyLandScale.Y * CollisionScale;
					PTriMeshGeomEd.scale.scale.z = CyLandScale.Z;
			...
			{
				UE_LOG(LogCyLand, Log, TEXT("UCyLandMeshCollisionComponent::OnCreatePhysicsState(): TriMesh invalid"));
			}
		}
#endif // WITH_PHYSX
	}
...
void UCyLandMeshCollisionComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
	Super::ApplyWorldOffset(InOffset, bWorldShift);
...
void UCyLandMeshCollisionComponent::DestroyComponent(bool bPromoteChildren/*= false*/)
{
	ACyLandProxy* Proxy = GetCyLandProxy();
	if (Proxy)
	{
...
#if WITH_EDITOR
void UCyLandHeightfieldCollisionComponent::UpdateHeightfieldRegion(int32 ComponentX1, int32 ComponentY1, int32 ComponentX2, int32 ComponentY2)
{
#if WITH_PHYSX
	if (IsValidRef(HeightfieldRef))
	{
		...
		FPhysicsCommand::ExecuteWrite(PhysActorHandle, [&](const FPhysicsActorHandle& Actor)
		{
			...
			//
			FVector CyLandScale = GetComponentToWorld().GetScale3D().GetAbs();
			// Create the geometry
			PxHeightFieldGeometry CyLandComponentGeom(HeightfieldRef->RBHeightfieldEd, PxMeshGeometryFlags(), CyLandScale.Z * LANDSCAPE_ZSCALE, CyLandScale.Y * CollisionScale, CyLandScale.X * CollisionScale);
	
			{
				...
				if (NumShapes > 1)
				{
					FPhysicsInterface::SetGeometry(PShapes[1], CyLandComponentGeom);
				}
			}
		});
	}
...
void UCyLandHeightfieldCollisionComponent::DestroyComponent(bool bPromoteChildren/*= false*/)
{
	ACyLandProxy* Proxy = GetCyLandProxy();
	if (Proxy)
	{
...
FBoxSphereBounds UCyLandHeightfieldCollisionComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	return CachedLocalBox.TransformBy(LocalToWorld);
}

void UCyLandHeightfieldCollisionComponent::BeginDestroy()
{
	HeightfieldRef = NULL;
...
void UCyLandMeshCollisionComponent::BeginDestroy()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		MeshRef = NULL;
...
void UCyLandHeightfieldCollisionComponent::RecreateCollision()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		HeightfieldRef = NULL;
...
void UCyLandHeightfieldCollisionComponent::SnapFoliageInstances(const FBox& InInstanceBox)
{
			...
			if (InstanceSet)
			{
				...
				for (int32 InstanceIndex : *InstanceSet)
				{
					...
					if (InInstanceBox.IsInside(TestLocation))
					{
						...
						World->LineTraceMultiByObjectType(Results, Start, End, FCollisionObjectQueryParams(ECollisionChannel::ECC_Visibility), FCollisionQueryParams(SCENE_QUERY_STAT(FoliageSnapToCyLand), true));
...
void UCyLandMeshCollisionComponent::RecreateCollision()
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		MeshRef = NULL;
...
void UCyLandHeightfieldCollisionComponent::Serialize(FArchive& Ar)
{
#if WITH_EDITOR
...
void UCyLandMeshCollisionComponent::Serialize(FArchive& Ar)
{
	Super::Serialize(Ar);
	...
	if (bCooked)
	{
		// triangle mesh cooked data should be serialized in UCyLandHeightfieldCollisionComponent
	}
...
#if WITH_EDITOR
void UCyLandHeightfieldCollisionComponent::PostEditImport()
{
...
void UCyLandHeightfieldCollisionComponent::PostEditUndo()
{
	Super::PostEditUndo();
...
bool UCyLandHeightfieldCollisionComponent::ComponentIsTouchingSelectionBox(const FBox& InSelBBox, const FEngineShowFlags& ShowFlags, const bool bConsiderOnlyBSP, const bool bMustEncompassEntireComponent) const
{
...
bool UCyLandHeightfieldCollisionComponent::ComponentIsTouchingSelectionFrustum(const FConvexVolume& InFrustum, const FEngineShowFlags& ShowFlags, const bool bConsiderOnlyBSP, const bool bMustEncompassEntireComponent) const
{
	if (ShowFlags.Landscape)
	{
...
bool UCyLandHeightfieldCollisionComponent::DoCustomNavigableGeometryExport(FNavigableGeometryExport& GeomExport) const
{
...
void UCyLandHeightfieldCollisionComponent::GatherGeometrySlice(FNavigableGeometryExport& GeomExport, const FBox& SliceBox) const
{
...
ENavDataGatheringMode UCyLandHeightfieldCollisionComponent::GetGeometryGatheringMode() const
{ 
	ACyLandProxy* Proxy = GetCyLandProxy();
	return Proxy ? Proxy->NavigationGeometryGatheringMode : ENavDataGatheringMode::Default;
}

void UCyLandHeightfieldCollisionComponent::PrepareGeometryExportSync()
{
	//check(IsInGameThread());
#if WITH_PHYSX
...
bool UCyLandMeshCollisionComponent::DoCustomNavigableGeometryExport(FNavigableGeometryExport& GeomExport) const
{
	check(IsInGameThread());
#if WITH_PHYSX
...
void UCyLandHeightfieldCollisionComponent::PostLoad()
{
	Super::PostLoad();
	...
	if (!HasAnyFlags(RF_ClassDefaultObject) && !IsPendingKill())
	{
		...
		if (ensure(CyLandProxy) && GIsEditor)
		{
			...
			{
				UE_LOG(LogCyLand, Warning, TEXT("UCyLandHeightfieldCollisionComponent RelativeLocation disagrees with its section base, attempted automated fix: '%s', %f,%f vs %f,%f."),
...
void UCyLandHeightfieldCollisionComponent::PreSave(const class ITargetPlatform* TargetPlatform)
{
...
#if WITH_EDITOR
void UCyLandInfo::UpdateAllAddCollisions()
{
	...
	if (GetCyLandProxy())
	{
		...
		{
			const UCyLandComponent* const Component = It.Value();
			if (ensure(Component))
			{
				...
				for (int32 i = 0; i < 8; ++i)
				{
					UCyLandComponent* NeighborComponent = XYtoComponentMap.FindRef(NeighborsKeys[i]);
...
void UCyLandInfo::UpdateAddCollision(FIntPoint CyLandKey)
{
	FCyLandAddCollision& AddCollision = XYtoAddCollisionMap.FindOrAdd(CyLandKey);
	...
	{
		CyLandKey + FIntPoint(-1, -1),
		CyLandKey + FIntPoint(+0, -1),
		CyLandKey + FIntPoint(+1, -1),
		CyLandKey + FIntPoint(-1, +0),
		CyLandKey + FIntPoint(+1, +0),
		CyLandKey + FIntPoint(-1, +1),
		CyLandKey + FIntPoint(+0, +1),
		CyLandKey + FIntPoint(+1, +1)
	...
	UCyLandHeightfieldCollisionComponent* NeighborCollisions[8];
	// Search for Neighbors...
	for (int32 i = 0; i < 8; ++i)
	{
		UCyLandComponent* Comp = XYtoComponentMap.FindRef(NeighborsKeys[i]);
		if (Comp)
		{
	...
	FIntPoint SectionBase = CyLandKey * ComponentSizeQuads;

	// Transform Height to Vectors...
	FTransform LtoW = GetCyLandProxy()->CyLandActorToWorld();
	AddCollision.Corners[0] = LtoW.TransformPosition(FVector(SectionBase.X                     , SectionBase.Y                     , CyLandDataAccess::GetLocalHeight(HeightCorner[0])));
	AddCollision.Corners[1] = LtoW.TransformPosition(FVector(SectionBase.X + ComponentSizeQuads, SectionBase.Y                     , CyLandDataAccess::GetLocalHeight(HeightCorner[1])));
	AddCollision.Corners[2] = LtoW.TransformPosition(FVector(SectionBase.X                     , SectionBase.Y + ComponentSizeQuads, CyLandDataAccess::GetLocalHeight(HeightCorner[2])));
	AddCollision.Corners[3] = LtoW.TransformPosition(FVector(SectionBase.X + ComponentSizeQuads, SectionBase.Y + ComponentSizeQuads, CyLandDataAccess::GetLocalHeight(HeightCorner[3])));
}

void UCyLandHeightfieldCollisionComponent::ExportCustomProperties(FOutputDevice& Out, uint32 Indent)
{
	if (HasAnyFlags(RF_ClassDefaultObject))
	{
		return;
...
void UCyLandHeightfieldCollisionComponent::ImportCustomProperties(const TCHAR* SourceText, FFeedbackContext* Warn)
{
...
void UCyLandMeshCollisionComponent::ExportCustomProperties(FOutputDevice& Out, uint32 Indent)
{
	if (HasAnyFlags(RF_ClassDefaultObject))
	{
		return;
...
void UCyLandMeshCollisionComponent::ImportCustomProperties(const TCHAR* SourceText, FFeedbackContext* Warn)
{
	if (FParse::Command(&SourceText, TEXT("CollisionHeightData")))
	{
		int32 NumHeights = FMath::Square(CollisionSizeQuads + 1);
...
UCyLandInfo* UCyLandHeightfieldCollisionComponent::GetCyLandInfo() const
{
...
ACyLandProxy* UCyLandHeightfieldCollisionComponent::GetCyLandProxy() const
{
	return CastChecked<ACyLandProxy>(GetOuter());
}

FIntPoint UCyLandHeightfieldCollisionComponent::GetSectionBase() const
{
	return FIntPoint(SectionBaseX, SectionBaseY);
}

void UCyLandHeightfieldCollisionComponent::SetSectionBase(FIntPoint InSectionBase)
{
	SectionBaseX = InSectionBase.X;
	SectionBaseY = InSectionBase.Y;
}

UCyLandHeightfieldCollisionComponent::UCyLandHeightfieldCollisionComponent(const FObjectInitializer& ObjectInitializer)
