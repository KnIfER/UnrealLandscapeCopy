...
#include "CyLandHeightfieldCollisionComponent.generated.h"
...
UCLASS(MinimalAPI, Within=CyLandProxy)
class UCyLandHeightfieldCollisionComponent : public UPrimitiveComponent
{
	GENERATED_UCLASS_BODY()

	/** List of layers painted on this component. Matches the WeightmapLayerAllocations array in the CyLandComponent. */
	UPROPERTY()
	TArray<UCyLandLayerInfoObject*> ComponentLayerInfos;
	...
	UPROPERTY()
	TLazyObjectPtr<UCyLandComponent> RenderComponent;
...
public:
	...
	UCyLandInfo* GetCyLandInfo() const;
	...
	CYLAND_API virtual void RecreateCollision();

#if WITH_EDITORONLY_DATA
	// Called from editor code to manage foliage instances on landscape.
	CYLAND_API void SnapFoliageInstances(const FBox& InInstanceBox);
#endif
};
