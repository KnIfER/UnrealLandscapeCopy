...
//
// FCyLandToolRetopologize
//

class FCyLandToolStrokeRetopologize : public FCyLandToolStrokeBase
{
public:
	FCyLandToolStrokeRetopologize(FEdModeCyLand* InEdMode, FEditorViewportClient* InViewportClient, const FCyLandToolTarget& InTarget)
	...
	void Apply(FEditorViewportClient* ViewportClient, FCyLandBrush* Brush, const UCyLandEditorObject* UISettings, const TArray<FCyLandToolInteractorPosition>& InteractorPositions)
	{
		...
		//CyLandInfo->Modify();
		//CyLandInfo->CyLandProxy->Modify();

		FVector DrawScale3D = CyLandInfo->DrawScale;
		// Tablet pressure
		...
		*/
		{
			...
			int32 ComponentSizeQuads = CyLandInfo->ComponentSizeQuads;
		...
		// Retopologize algorithm...
		{
			...
			// Calculate Average...
			TArray<UCyLandComponent*> ComponentArray; // Ptr to component
			...
			for (int32 ComponentIndexY = ComponentIndexY1; ComponentIndexY <= ComponentIndexY2; ComponentIndexY++)
			{
				for (int32 ComponentIndexX = ComponentIndexX1; ComponentIndexX <= ComponentIndexX2; ComponentIndexX++)
				{
					UCyLandComponent* Comp = CyLandInfo->XYtoComponentMap.FindRef(FIntPoint(ComponentIndexX, ComponentIndexY));
			...
			for (int32 Y = Y1; Y < Y2 - 1; ++Y)
			{
				...
				while (FMath::Abs(TotalLineArea - TargetLineArea) > AreaErrorThreshold)
				{
					...
					{
						UCyLandComponent* Comp = ComponentArray[X - X1 + (Y - Y1)*(X2 - X1 + 1)];
						if (Comp != NULL) // valid
						{
							const FMatrix LocalToWorld = Comp->GetRenderMatrix();
							FVector P[4];
...
protected:
	FCyLandXYOffsetCache<false> Cache;
};

class FCyLandToolRetopologize : public FCyLandToolBase<FCyLandToolStrokeRetopologize>
{
public:
	FCyLandToolRetopologize(FEdModeCyLand* InEdMode)
		: FCyLandToolBase<FCyLandToolStrokeRetopologize>(InEdMode)
	{
	}

	virtual const TCHAR* GetToolName() override { return TEXT("Retopologize"); }
	virtual FText GetDisplayName() override { return NSLOCTEXT("UnrealEd", "CyLandMode_Retopologize", "Retopologize"); }
...
void FEdModeCyLand::InitializeTool_Retopologize()
{
	auto Tool_Retopologize = MakeUnique<FCyLandToolRetopologize>(this);
	...
	CyLandTools.Add(MoveTemp(Tool_Retopologize));
}
