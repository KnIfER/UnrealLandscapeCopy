...
//
// FCyLandToolErosionBase
//
class FCyLandToolStrokeErosionBase : public FCyLandToolStrokeBase
{
public:
	FCyLandToolStrokeErosionBase(FEdModeCyLand* InEdMode, FEditorViewportClient* InViewportClient, const FCyLandToolTarget& InTarget)
		: FCyLandToolStrokeBase(InEdMode, InViewportClient, InTarget)
		, HeightCache(InTarget)
		, WeightCache(InTarget)
		, bWeightApplied(InTarget.TargetType != ECyLandToolTargetType::Heightmap)
	{
	}
...
class FCyLandToolErosionBase : public FCyLandToolBase<TStrokeClass>
{
public:
	FCyLandToolErosionBase(FEdModeCyLand* InEdMode)
...
class FCyLandToolStrokeErosion : public FCyLandToolStrokeErosionBase
{
public:
	FCyLandToolStrokeErosion(FEdModeCyLand* InEdMode, FEditorViewportClient* InViewportClient, const FCyLandToolTarget& InTarget)
		: FCyLandToolStrokeErosionBase(InEdMode, InViewportClient, InTarget)
	{
	}
					...
					if (BrushValue > 0.0f)
					{
						...
						if (SlopeTotal > 0)
						{
							float Softness = 1.0f;
							{
								...
								{
									UCyLandLayerInfoObject* LayerInfo = CyLandInfo->Layers[Idx].LayerInfoObj;
									if (LayerInfo)
									{
		...
		for (int32 Y = BrushInfo.GetBounds().Min.Y; Y < BrushInfo.GetBounds().Max.Y; Y++)
		{
			...
			for (int32 X = BrushInfo.GetBounds().Min.X; X < BrushInfo.GetBounds().Max.X; X++)
			{
				...
				if (BrushValue > 0.0f)
				{
					...
					float PaintAmount = NoiseModeConversion((ECyLandToolNoiseMode)UISettings->ErosionNoiseMode, NoiseParam.NoiseAmount, NoiseParam.Sample(X, Y));
					HeightData[(X - X1) + (Y - Y1)*(1 + X2 - X1)] = FCyLandHeightCache::ClampValue(HeightData[(X - X1) + (Y - Y1)*(1 + X2 - X1)] + PaintAmount);
				}
			}
		}
...
class FCyLandToolErosion : public FCyLandToolErosionBase<FCyLandToolStrokeErosion>
{
public:
	FCyLandToolErosion(FEdModeCyLand* InEdMode)
		: FCyLandToolErosionBase(InEdMode)
	{
	}

	virtual const TCHAR* GetToolName() override { return TEXT("Erosion"); }
	virtual FText GetDisplayName() override { return NSLOCTEXT("UnrealEd", "CyLandMode_Erosion", "Erosion"); };

};

//
// FCyLandToolHydraErosion
//

class FCyLandToolStrokeHydraErosion : public FCyLandToolStrokeErosionBase
{
public:
	FCyLandToolStrokeHydraErosion(FEdModeCyLand* InEdMode, FEditorViewportClient* InViewportClient, const FCyLandToolTarget& InTarget)
		: FCyLandToolStrokeErosionBase(InEdMode, InViewportClient, InTarget)
	{
	}
	...
	void Apply(FEditorViewportClient* ViewportClient, FCyLandBrush* Brush, const UCyLandEditorObject* UISettings, const TArray<FCyLandToolInteractorPosition>& InteractorPositions)
	{
		...
		for (int32 Y = BrushInfo.GetBounds().Min.Y; Y < BrushInfo.GetBounds().Max.Y; Y++)
		{
			...
			for (int32 X = BrushInfo.GetBounds().Min.X; X < BrushInfo.GetBounds().Max.X; X++)
			{
				...
				if (BrushValue >= 1.0f)
				{
					float PaintAmount = NoiseModeConversion((ECyLandToolNoiseMode)UISettings->RainDistMode, NoiseParam.NoiseAmount, NoiseParam.Sample(X, Y));
...
class FCyLandToolHydraErosion : public FCyLandToolErosionBase<FCyLandToolStrokeHydraErosion>
{
public:
	FCyLandToolHydraErosion(FEdModeCyLand* InEdMode)
		: FCyLandToolErosionBase(InEdMode)
	{
	}

	virtual const TCHAR* GetToolName() override { return TEXT("HydraErosion"); } // formerly HydraulicErosion
	virtual FText GetDisplayName() override { return NSLOCTEXT("UnrealEd", "CyLandMode_HydraErosion", "Hydraulic Erosion"); };
...
//
void FEdModeCyLand::InitializeTool_Erosion()
{
	auto Tool_Erosion = MakeUnique<FCyLandToolErosion>(this);
	...
	CyLandTools.Add(MoveTemp(Tool_Erosion));
}

void FEdModeCyLand::InitializeTool_HydraErosion()
{
	auto Tool_HydraErosion = MakeUnique<FCyLandToolHydraErosion>(this);
	...
	CyLandTools.Add(MoveTemp(Tool_HydraErosion));
}
